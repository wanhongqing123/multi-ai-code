...
//
// **名字里是 Adapter 不是 Server，这是有意的。
// ** 叫 MaiHttpServer 时几乎每个人第一眼都以为"这个 agent 是个服务端"——不是。
// 产物是 maiagent 这个库，
// HTTP 只是为了让说不了 C++ 的调用方（现在的 Electron 界面是 JS 写的）能连上来而临时贴的一层壳。
//
// Qt 桌面 / iOS / Android / 嵌入式全都直接链 maiagent，一行 HTTP 都不过。
// Qt 界面就位之后这整个目录可以删掉，核心一行都不用改。
//
// 顺带注意别和 libcurl 搞混：核心里的 libcurl 是 HTTP **客户端**（去调大模型，摘不掉）；
// 这里是 HTTP **服务端**（给界面连，可摘）。两者方向相反。
//
