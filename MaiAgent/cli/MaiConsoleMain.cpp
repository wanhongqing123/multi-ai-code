...
// ── 它首先是一个证明 ──────────────────────────────────────────────
//
// CMake 里这个目标只链 `maiagent`，不链 `maiagent_http_adapter`。所以它能编译、能跑，
// 就说明核心真的不认识 HTTP 服务端。规范第 9 节那句"adapters/ 和 cli/ 是可摘的壳"
// 以前只是一句文档，现在由构建系统盯着：哪天有人把 httplib 或者 JSON 漏进核心，
// 这个目标会当场链不过。
//
// 第二层作用是它本身就能用。Qt 界面就位之前调试 agent 不用再起三个进程（桥 + Electron + 模型），
// 跑这一个就行。移动端和嵌入式将来的调用方式和这里一模一样：构造 MaiAgent，
// 订阅事件总线，submit 操作。
//
// ── 和 maiagent-bridge 的区别 ────────────────────────────────────
//
//   maiagent-bridge    核心 + HTTP 适配器，给说不了 C++ 的调用方用。REST + SSE。
//   maiagent-console   只有核心，进程内直接订阅事件总线。不出现 HTTP / JSON / socket。
//
// ── 为什么不做成 TUI ────────────────────────────────────────────
//
