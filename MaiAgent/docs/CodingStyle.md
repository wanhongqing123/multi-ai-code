...
**这条规则第一次只在工具那条路径上落实了，正文和思考过程两条漏着，
半年后拿真模型跑才发现。** 而漏掉的代价比"拉不到"严重得多：

正文和思考过程的增量事件 `field` 都是 `"text"`，订阅方唯一能分清的办法
就是拿 partId 去查这个片段是 `MaiTextPart` 还是 `MaiReasoningPart`。
查不到就只能按正文渲染——于是模型的**思考草稿被当成答案打给了用户**，
而且和真答案直接粘在一起，中间连个换行都没有。

假模型不发 `reasoning_content`，所以端到端一路全绿。接上真实的 GLM-5.3
第一句话就露馅了。推论有两条：

- 广播一个 id 之前，先问"订阅方拿这个 id 去查，查得到吗"。
  查不到的话，它要靠什么分辨这是什么东西？
- 假模型要照着**真模型实际发的东西**造，不是照着自己的理解造。
  这一条和第 13 节是同一件事——观测看不见的地方，绿色不说明任何问题。
