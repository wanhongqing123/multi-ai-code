...
| 11 | 注释写"为什么"；**改名时把注释一起改** | 8 |
| 12 | 核心不认识 HTTP 服务端；公开头里不出现 JSON / HTTP / Qt | 9 |
| 13 | **先落库，再广播** | 10 |
| 14 | 闸门的兜底方向**永远是"不放行"** | 11 |
| 15 | 回灌给模型的话**必须是真的** | 12 |
| 16 | 碰文件走系统 API，**不用 `std::filesystem` / `fstream`** | 12.5 |
| 17 | 线程要有名字；事件处理函数里不许做慢活（有守卫） | 12.6 |
| 18 | 下结论前先问"这套观测能不能看见目标"；改断言做红→绿对照 | 13 |
