...
// "这个线程现在不许做慢活"。
//
// 形状参考 chromium 的 base/threading/thread_restrictions.h。它把这件事
// 说得很清楚：
//
...
//
// 光靠注释是守不住的。以后有人在事件处理函数里顺手读个文件、写一次库，
// 表现出来不是崩溃，是"模型吐字变卡了"——这种症状没人会联想到事件总线。
//
// 现在 MaiEventBus::publish 调处理函数时会套上 MaiScopedDisallowBlocking，
// 而 MaiFileSystem 的读写和 SQLite 的写入都会先问一句
// maiAssertBlockingAllowed()。违反了就当场炸，而不是留到线上变成玄学卡顿。
//
// ── 为什么不是只在 Debug 构建里查 ──────────────────────────────
//
// 这个项目的测试是 Release 构建的（见 CMakeLists）。只在 Debug 里查等于
// 我们自己的测试永远查不到，那这套东西就白写了。检查本身是读一个
// thread_local 的 bool，和它守着的那些系统调用比，开销可以忽略。
...
//
// 终止而不是返回错误，是因为这属于**编程错误**，和越界访问同一类：
// 返回错误的话调用方多半会忽略，而症状会飘到很远的地方去。
...
//
// 对应 chromium 的 ScopedAllowBlocking。用它之前先想清楚——
// 大多数情况下正确的做法是**把慢活挪出去**（塞进队列，换个线程做），
// 而不是在这里声明豁免。
