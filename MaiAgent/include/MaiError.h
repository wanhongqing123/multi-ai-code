...
//
// ── 为什么不用异常 ──────────────────────────────────────────────
// 这个库要被链进 Qt 桌面、iOS / Android、嵌入式。那些环境里异常要么被
// 关掉（-fno-exceptions），要么跨 ABI 边界传不过去。返回值到处都能用。
//
// ── 为什么要错误码，光有字符串不行 ──────────────────────────────
// 这一版之前全靠字符串传错误，调用方只能靠 substring 去猜出了什么事。
// 界面想区分"没配模型"（该弹设置页）和"网络断了"（该提示重试）就没办法，
// HTTP 适配器也没法把它映射成合适的状态码。
//
// 现在两样都有：**码给机器看，文字给人看**。
...
enum class MaiErrorCode {
    ...
    // 用户主动中断，或者授权超时。**这不是故障**——
    // 界面不该弹错误，已经吐出来的内容也要照常保留。
    // MaiTurnRunner::finish 专门判了这一条。
    Canceled,
    Internal,  // 剩下的。落库失败、curl 初始化失败之类
};

// 转成 "not_found" 这类线上字符串。HTTP 适配器把它放进错误响应的 code 字段，
// 界面据此分支而不是去匹配 message 的文字。
...
public:
    ...
    // 方便写 `if (error) { ... }`。
    //
    // 注意这里的方向：**有错时为真**，和 MaiResult 正好相反
    // （那边是"成功时为真"）。两个类型混着用的地方容易看岔，
    // 拿不准就写 hasError() / isOk()，别图短。
...
// 不支持 map / and_then 那套，也不管引用类型和 void。那些会引入一堆模板
// 复杂度，而这个工程里的用法就三种——submit 的返回、打开数据库、
// 以后可能的工厂函数。
//
// T 必须能默认构造：出错时 mValue 是默认构造出来的，读它没有意义但也
// 不会是未定义行为。
//
// 这是编码规范第 5 节说的**例外**：模板的实例化需要看得见定义，
// 所以实现只能留在头文件里。
template <typename T>
class MaiResult {
public:
