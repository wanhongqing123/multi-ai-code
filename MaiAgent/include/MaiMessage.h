...
//
// 这也是为什么这里没有任何 include 指向 MaiSessionStore 或 MaiModelClient：
// 依赖方向是单向的，它们认识 MaiMessage，MaiMessage 不认识它们。
//
// ── 一条 assistant 消息长什么样 ────────────────────────────────
...
//
// 界面就按这个顺序渲染。片段顺序 = 生成顺序，落库靠 id 的字典序还原
// （见 MaiIdGenerator.h 对单调性的许诺）。
...
//
// **不回灌给模型**——它是草稿，回灌会污染下一轮上下文。
// MaiContextBuilder 组装历史时会跳过这种片段，那里有一条用例盯着。
//
// 界面上通常折叠显示，所以它和 MaiTextPart 必须是**两个片段**而不是
// 拼在一起：混成一个的话，界面没办法只折叠思考那一段。
...
// 一次工具调用走到哪一步了。
//
// 这几个值和 SQLite 里 parts.state 列的数字绑死（见 MaiSqliteStore.cpp），
// **不要改它们的顺序**，库里已经存下的行不会跟着变。加新值往后追加。
enum class MaiToolState {
    // 等用户授权。会改东西的工具（write）在闸门那儿停下时就是这个状态，
    // 界面靠它显示"等待授权"。见 MaiPermission.h。
    Pending,
    Running,    // 正在执行
...
// 一次工具调用。从"模型说要调"到"结果回来了"，全程是**同一个片段**在
// 原地更新状态，不是每一步新建一个——界面据此做增量刷新，新建的话
// 工具卡会闪。
struct MaiToolPart {
    ...
    std::string callId;
    // 参数的 JSON 原文。核心**不解析**它，原样交给工具实现去解——
    // 这样加新工具不用动核心的任何代码。
    std::string input;
    // 成功时的输出。会被塞进上下文喂给模型，所以有长度上限
    // （见 MaiFileTools.cpp 里的 kMaxOutputBytes），截断时会明确告诉模型。
    std::string output;
    // 失败原因。**失败时 output 里放的是同一句话**——模型要知道失败了
    // 才能换个做法，而它只读 tool 消息的 content。
    std::string error;
...
struct MaiMessagePart {
    // prt_...，**创建后永不改变**。
    //
    // 界面靠它做增量更新：message.part.delta 事件只带 partId 和这次新增的
    // 内容，界面找到对应的片段往后追加。id 变了就等于换了个片段，
    // 界面会当成新的渲染一遍。
    std::string id;
...
// 只有两种角色。
//
// 没有 System：系统提示词是 MaiContextBuilder 在组装请求时**临时加**的，
// 不属于对话历史，也不该被存下来或显示给用户。
// 没有 Tool：工具结果是 assistant 消息里的一个片段（MaiToolPart），
...
struct MaiMessage {
    ...
    //
    // 用在两个地方：给会话起标题（取用户第一句话），以及把历史组装成
    // 上下文时还原 assistant 说过的话。两处都只要"说出来的话"，
    // 不要草稿也不要工具输出。
