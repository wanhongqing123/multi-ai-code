...
public:
    ...
    //
    // 为什么写接口返回 void、错误另开一个查询：putMessage 在流式的热路径上
    // 被反复调用，每个调用点都 if 一下会把代码淹掉，而且那些地方也做不了
    // 什么补救。但**失败不能无声无息**——磁盘满了还假装存上了，是用户第二天
    // 打开发现对话没了的那种 bug。
    //
...
//
// 传 ":memory:" 可以拿到一个不落盘的 SQLite 库——那和 makeMaiMemoryStore()
// 不是一回事：后者是纯 STL 的哈希表，前者仍然走完整的 SQL 路径，
// 测试里用它来验证 SQL 本身，而不用碰磁盘。
//
// 打不开（路径非法、磁盘只读、文件不是数据库）就返回错误，不返回一个
// 半死的对象。这是唯一一处把存储错误做成返回值的地方，因为构造失败时
// 调用方**能**做点什么：换个路径，或者退回内存存储。
