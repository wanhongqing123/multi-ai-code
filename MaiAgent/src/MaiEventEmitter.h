...
// 把"构造事件并发布"这件事收在一处。
//
// 之前 MaiAgent::Runtime 里散着四个发事件的私有方法，每个都在手工填
// id/time/type 那几个字段——加一种事件就要再抄一遍，抄漏一个字段不会编译报错，
// 只会在界面上表现成某些更新丢了。
//
...
public:
    ...
    // 增量。**只带这次新增的内容**，不重推全量——流式期间每秒几十条，全量重推是纯烧 CPU。
    ...
    // 有一次工具调用在等用户点头。partId 指向那个工具 part，界面从 part 上就能拿到工具名和参数，
    // 不用这个事件再带一份。
