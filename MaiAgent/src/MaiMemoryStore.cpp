...
public:
    void putSession(const MaiSession& s) override {
    ...
    std::vector<MaiSession> listSessions() const override {
        ...
        std::sort(out.begin(), out.end(),
                  [](const MaiSession& a, const MaiSession& b) { return a.updated > b.updated; });
        return out;
    }
    ...
    bool mutateSession(const std::string& id, const std::function<void(MaiSession&)>& fn) override {
    ...
    void putMessage(const std::string& sessionId, const MaiMessage& m) override {
        std::lock_guard<std::mutex> lock(mu_);
        auto& list = messages_[sessionId];
        // 同 id 视为更新：流式期间同一条消息会被反复写回。
        auto it = std::find_if(list.begin(), list.end(),
                               [&](const MaiMessage& x) { return x.id == m.id; });
        if (it != list.end()) {
            *it = m;
        } else {
            list.push_back(m);
    ...
    std::vector<MaiMessage> listMessages(const std::string& sessionId) const override {
        std::lock_guard<std::mutex> lock(mu_);
        auto it = messages_.find(sessionId);
...
private:
    ...
    std::unordered_map<std::string, MaiSession> sessions_;
    std::unordered_map<std::string, std::vector<MaiMessage>> messages_;
};
