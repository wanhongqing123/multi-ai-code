...
// 为什么不用假 HTTP 服务端：
//
// MaiModelClient 本来就是个抽象接口，MaiAgent 构造时接的就是它。测 agent 的行为（一轮的生命周期、
// 工具循环、权限闸门）根本不需要经过传输层。以前那三个测试各自起一个 httplib 服务端，
// 于是每条断言都要走
//   组装 MaiModelRequest -> 序列化成 JSON -> TCP -> curl -> SSE 解析 -> 回调
// 一圈。想断言"第二轮带上了上一轮的回答"，
// 得去 JSON 里翻`body["messages"][1]["content"]`——而那句话本来就在 MaiModelRequest 这个结构体里放着
// 。
//
// 换成直接实现接口之后：
...
// 而 line_buffer 那种纯解析逻辑是直接喂字节的单元测试（codex-rs/ollama/src/line_buffer_tests.rs）。
class MaiFakeModelClient final : public MaiModelClient {
public:
    // 一次应答的剧本。
    struct Turn {
        // 分片吐出的正文。分片是为了让上层能收到多条 delta 事件——界面就是靠这些增量渲染的，
        // 一次给全就测不到拼接。
        ...
        // 每个分片之间停多久。只在需要"还在跑"这个窗口的用例里设（测 busy 拒绝、测中断），默认 0。
    ...
    // 不按序号取剧本，每次请求都回同一个。
    //
    // 并发用例要这个：4 个会话同时问，按序号取的话它们会各拿到剧本里不同的一条，
    // 而那个用例想验证的恰恰是"4 个都拿到一样的完整回答"。
...
private:
    // 剧本是共享状态：同一个客户端会被连着调好几次（工具循环里一轮有多次请求），
    // 而这些调用可能来自不同线程。
