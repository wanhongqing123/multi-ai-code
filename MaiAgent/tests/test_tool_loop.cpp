...
struct Workspace {
  ...
  std::string utf8_root() const { return root.u8string(); }
};
...
std::unique_ptr<Agent> make_agent(const FakeModel& model) {
  ...
  return std::make_unique<Agent>(make_memory_store(), make_model_client(cfg),
                                 std::move(tools), opts);
}
...
void test_tool_loop_closes() {
  ...
  // 6. 界面能看到工具卡的状态变化
  std::size_t part_updates = 0;
  for (const auto& e : rec.all())
    if (e.type == EventType::MessagePartUpdated) ++part_updates;
  CHECK(part_updates >= 2);  // 至少 running 和 completed 各一次
}
...
void test_tool_error_is_fed_back() {
  ...
  if (msgs.size() == 2)
    for (const auto& p : msgs[1].parts)
      if (const auto* t = std::get_if<ToolPart>(&p.body)) tp = t;
  CHECK(tp && tp->state == ToolState::Error);
}
...
void test_unknown_tool_does_not_kill_the_turn() {
  ...
  CHECK(model.request_count() == 2);
  const auto msgs = agent->messages(sid);
  bool has_final_text = false;
  if (msgs.size() == 2)
    for (const auto& p : msgs[1].parts)
      if (const auto* t = std::get_if<TextPart>(&p.body))
        if (t->text.find("用错工具") != std::string::npos) has_final_text = true;
  CHECK(has_final_text);
}
...
void test_path_escape_through_model() {
  ...
  CHECK(tp != nullptr);
  if (tp) {
    CHECK(tp->state == ToolState::Error);
...
void test_iteration_cap() {
  ...
  Agent::Options opts;
  opts.default_model = "glm-5.3";
  opts.max_tool_iterations = 3;  // 调小便于测试
  Agent agent(make_memory_store(), make_model_client(cfg), std::move(tools), opts);
  ...
  // 到上限就停，不能无限烧钱
  CHECK(model.request_count() == 3);
  // 而且要明确告诉用户停在哪儿了，不是悄悄结束让人以为跑完了
  bool told_user = false;
  for (const auto& e : rec.all())
    if (e.type == EventType::SessionError && e.detail.find("上限") != std::string::npos)
      told_user = true;
  CHECK(told_user);
}
