...
- 输入区提供图片、文本文件、语音转文字、权限和模型配置入口。未配置模型可直接进入设置；`glm-5.3` 与 `glm-5.3-flash` 可在输入区直接切换。
...
提供手机沙箱工作区内的读取、写入、编辑、补丁、目录查找、文本搜索、图片查看，以及网络读取、时间和提问。文本导入限 5MB 以内的 UTF-8 文本/代码；图片导入限 20MB，支持 JPEG、PNG、WebP、GIF，系统可解码的其他图片会先转成 JPEG。图片字节以 Base64 Data URL 交给支持视觉输入的模型，本地路径不会作为模型可访问地址发送。
...
新增底部 AI 助手入口，具备与 iOS 相同的会话、模型/权限配置、流式回复、停止、图片与文本导入、拍照、语音转文字、授权及提问功能。输入区可直接切换 `glm-5.3` 与 `glm-5.3-flash`；文本模型会在发送图片前提示切换，已选图片不会丢失。正文复用 IM 的 Markwon 渲染与第 2 项完成的 iOS 字体参数；思考和工具详情可以展开，图片消息可从稳定工作区恢复缩略图。最近 50 条 AI 消息先进入列表，用户可按需显示更早消息。
