...
enum TIMGroupReportType {
    ...
    // 申请加群的处理通知（发给所有管理员和群主）
    kTIMGroupReport_AddResponseToAdmin,
    // 群申请列表已读状态变更通知（多端同步）
    kTIMGroupReport_PendencyReadInfoChanged,
};
 ...
 *
 *         } else if (elem_type == TIMElemType::kTIMElem_Stream) { // 流式消息
 *
 *         }
 *     }
 * }
...
//------------------------------------------------------------------------------
// 7.9 StreamElem (流式消息元素)
// @note
//  - 客户端只能接收流式消息，不能发送
// string, 只读, 流式消息 markdown 内容
static const char* kTIMStreamElemMarkdown = "stream_elem_markdown";
// string, 只读, 流式消息二进制数据
static const char* kTIMStreamElemData = "stream_elem_data";
// bool, 只读, 流式消息是否结束
static const char* kTIMStreamElemIsStreamEnded = "stream_elem_is_stream_ended";

//------------------------------------------------------------------------------
// 7.10 GroupMessageAtALL(@ 群里所有人的参数)
...
// string, 只读, 群申请列表已读状态变更通知（多端同步）
static const char* kTIMGroupReportElemPendencyReadInfo = "group_report_elem_pendency_read_info";
...
// array @ref GroupMemberInfo, 只读, 被操作者群内资料(只有 kTIMGroupReportElemReportType 为 kTIMGroupReport_AddResponseToAdmin 时有值)
static const char* kTIMGroupReportElemTargetGroupMemberInfoArray = "group_report_elem_target_group_memberinfo_array";
...
//------------------------------------------------------------------------------
// 7.15 MsgBatchSendResult (消息群发接口的返回)
...
//------------------------------------------------------------------------------
// 7.16 DownloadElemParam (下载元素接口的参数)
...
//------------------------------------------------------------------------------
// 7.17 MsgDownloadElemResult (下载元素接口的返回)
// uint, 只读, 当前已下载的大小
