...
//
// 得自己拦 keyPressEvent：QTextEdit 默认把 Enter 当换行，而这个框是三行高的，
// 不拦的话用户每轮都得去点发送按钮。
...
// ── 思考条 ──────────────────────────────────────────────────────
//
// **不是气泡**——气泡表示"谁说了句话"，这个表示"它在想"。
//
// 默认折叠：真实的推理模型思考期能有十几秒，展开会把答案推出屏幕。
// 但折叠不等于没有：那段时间里一个正文字都不会来，屏幕上完全没反应的话和卡死分不开，
// 所以收着的时候也要有个东西在动。这是拿真的 GLM-5.3 跑出来的，假模型测不到。
class AgentChatPanel::ThinkingStrip final : public QFrame {
public:
    explicit ThinkingStrip(QWidget* parent = nullptr) : QFrame(parent) {
        setStyleSheet(UiZoom::scaleQss(
            QStringLiteral("QFrame{background:%1;border-radius:6px;}").arg(kLineSoft)));
        setCursor(Qt::PointingHandCursor);

        auto* column = new QVBoxLayout(this);
        column->setContentsMargins(UiZoom::s(9), UiZoom::s(4), UiZoom::s(9), UiZoom::s(4));
        column->setSpacing(UiZoom::s(4));

        caption_ = makeLabel(QStringLiteral("▸ 思考中"), 12, kInkSoft);
...
private:
    void refreshCaption() {
        ...
        if (ticker_->isActive()) {
            dots_ = (dots_ + 1) % 4;
            caption_->setText(caret + QStringLiteral(" 思考中") + QString(dots_, QChar('.')));
        } else {
            caption_->setText(caret + QStringLiteral(" 思考了 %1 秒").arg(seconds_));
        }
    }
...
// ── 工具卡 ──────────────────────────────────────────────────────
//
// 也不是气泡：它表示"发生了一件事"。做成气泡的话，一轮对话读起来像三个人在说话。
//
// 等授权时它**就地**长出按钮，不弹对话框：弹窗会打断流式输出的阅读，
...
struct AgentChatPanel::Runtime {
    ...
    //（重画会让正在长的气泡闪）。
    QHash<QString, QLabel*> textBubbles;
    QHash<QString, ThinkingStrip*> thinkingStrips;
...
AgentChatPanel::AgentChatPanel(AgentController& controller, QWidget* parent)
    ...
    // ---- 头部：只有标题 ----
    // 工作目录和模型不放这儿，放输入区上方——那是你要动手的地方，
    // 而目录是"它能碰到什么"的边界，越靠近输入越好。
    ...
    headRow->setContentsMargins(UiZoom::s(16), UiZoom::s(11), UiZoom::s(16), UiZoom::s(11));
    ...
    // ---- 对话区顶上：攒了多少 + 清空 ----
    // 把这个数摆出来是有理由的：协议是无状态的，历史每一轮都要全量重发，
    // 聊得越久每句话越贵越慢。藏进菜单的话用户不会主动去点——
    // 他感觉不到自己在为一个月前的对话付钱。
    auto* contextLine = new QWidget(this);
    contextLine->setStyleSheet(UiZoom::scaleQss(
        QStringLiteral("QWidget{background:#f7f9fc;border-bottom:1px solid %1;}").arg(kLineSoft)));
    auto* contextRow = new QHBoxLayout(contextLine);
    contextRow->setContentsMargins(UiZoom::s(16), UiZoom::s(5), UiZoom::s(16), UiZoom::s(5));
    contextRow->addStretch(1);
    runtime_->contextSize = makeLabel(QStringLiteral("还没开始"), 11, kInkFaint);
    runtime_->contextSize->setWordWrap(false);
    contextRow->addWidget(runtime_->contextSize);
    ...
    contextRow->addWidget(clearButton);
    contextRow->addStretch(1);
    root->addWidget(contextLine);

    // ---- 对话流 ----
    runtime_->scroll = new QScrollArea(this);
    runtime_->scroll->setWidgetResizable(true);
    runtime_->scroll->setFrameShape(QFrame::NoFrame);
    runtime_->scroll->setStyleSheet(QStringLiteral("QScrollArea{background:#fbfcfe;border:none;}"));
    auto* streamHost = new QWidget;
    streamHost->setStyleSheet(QStringLiteral("background:#fbfcfe;"));
    runtime_->stream = new QVBoxLayout(streamHost);
    runtime_->stream->setContentsMargins(UiZoom::s(16), UiZoom::s(16), UiZoom::s(16),
                                         UiZoom::s(16));
    runtime_->stream->setSpacing(UiZoom::s(10));
    ...
    composer->setStyleSheet(UiZoom::scaleQss(
        QStringLiteral("QWidget{background:#ffffff;border-top:1px solid %1;}").arg(kLine)));
    auto* composerColumn = new QVBoxLayout(composer);
    composerColumn->setContentsMargins(0, 0, 0, UiZoom::s(10));
    composerColumn->setSpacing(0);

    // 上下文条：目录和模型一直摆在那儿。跟人聊天不需要知道"在哪个目录"，
    // agent 必须——那是它能碰到什么的边界。
    auto* contextBar = new QWidget(composer);
    contextBar->setStyleSheet(UiZoom::scaleQss(
        QStringLiteral("QWidget{background:#fafbfd;border-bottom:1px solid %1;}").arg(kLineSoft)));
    auto* contextBarRow = new QHBoxLayout(contextBar);
    contextBarRow->setContentsMargins(UiZoom::s(14), UiZoom::s(6), UiZoom::s(14), UiZoom::s(6));
    contextBarRow->setSpacing(UiZoom::s(9));
    ...
    contextBarRow->addWidget(runtime_->dirChip);
    contextBarRow->addWidget(runtime_->modelChip);
    contextBarRow->addStretch(1);
    auto* settingsHint = makeLabel(QStringLiteral("在设置里改"), 11, kInkFaint);
    settingsHint->setWordWrap(false);
    contextBarRow->addWidget(settingsHint);
    composerColumn->addWidget(contextBar);

    // 输入框三行起。一行高的框会逼着人把任务写短，而任务写短的直接后果是它猜错。
    runtime_->editor = new PromptEdit(composer);
    runtime_->editor->setPlaceholderText(QStringLiteral("交给它做点什么…"));
    runtime_->editor->setFixedHeight(UiZoom::s(76));
    runtime_->editor->setStyleSheet(UiZoom::scaleQss(
        QStringLiteral("QTextEdit{border:1px solid %1;border-radius:7px;padding:7px 10px;"
    ...
    auto* editorWrap = new QHBoxLayout;
    editorWrap->setContentsMargins(UiZoom::s(14), UiZoom::s(10), UiZoom::s(14), 0);
    editorWrap->addWidget(runtime_->editor);
    composerColumn->addLayout(editorWrap);

    auto* foot = new QHBoxLayout;
    foot->setContentsMargins(UiZoom::s(14), UiZoom::s(8), UiZoom::s(14), 0);
    foot->setSpacing(UiZoom::s(12));
    for (const QString& affordance : {QStringLiteral("@ 引用文件"), QStringLiteral("/ 命令")}) {
        auto* label = makeLabel(affordance, 11, kInkSoft);
    ...
    connect(&controller, &AgentController::turnFinished, this, [this](const QString&) {
        setRunning(false);
        for (ThinkingStrip* strip : runtime_->thinkingStrips) strip->settle();
        refreshContextSize();
    });
...
void AgentChatPanel::openSession(const QString& sessionId) {
    ...
    if (runtime_->controller->agent().getSession(toUtf8(runtime_->sessionId), session)) {
        ...
        // 不用文件夹 emoji：这台机器的界面字体里不一定有那个字形，
        // 渲染出来是个豆腐块，比没有还糟。
        runtime_->dirChip->setText(QStringLiteral("工作目录 ") + QDir(directory).dirName());
...
void AgentChatPanel::reloadFromStore() {
    ...
    runtime_->textBubbles.clear();
    runtime_->thinkingStrips.clear();
    ...
    for (const MaiMessage& message : runtime_->controller->agent().listMessages(
        ...
        for (const MaiMessagePart& part : message.parts) {
            ...
            if (const auto* text = std::get_if<MaiTextPart>(&part.body)) {
                textBubbleFor(partId)->setText(fromUtf8(text->text));
            } else if (const auto* reasoning = std::get_if<MaiReasoningPart>(&part.body)) {
                ThinkingStrip* strip = thinkingStripFor(partId);
                strip->append(fromUtf8(reasoning->text));
                strip->settle();  // 历史里的思考早就结束了，别让它转圈
...
void AgentChatPanel::addToStream(QWidget* widget, Qt::Alignment alignment) {
    ...
    // 的 sizeHint 是**不换行**那一行的宽度；再被 maximumWidth 卡到 520，
    // 高度却还是一行——结果就是长文本被硬生生截断。踩过一次，两个气泡都只显示半句。
    //
    // 正确做法是给每条消息包一层横向行：行本身撑满宽度，气泡在行里靠左或靠右，
    // 这样 heightForWidth 才算得对。
...
void AgentChatPanel::appendUserBubble(const QString& text) {
    ...
    bubble->setStyleSheet(UiZoom::scaleQss(
        QStringLiteral("QLabel{background:%1;color:#ffffff;border-radius:9px;padding:8px 12px;}")
            .arg(kAccent)));
    bubble->setMaximumWidth(UiZoom::s(460));
...
QLabel* AgentChatPanel::textBubbleFor(const QString& partId) {
    auto found = runtime_->textBubbles.constFind(partId);
    if (found != runtime_->textBubbles.constEnd()) return found.value();

    auto* bubble = makeLabel(QString(), 13, kInk);
    bubble->setStyleSheet(UiZoom::scaleQss(
        QStringLiteral("QLabel{background:#ffffff;color:%1;border:1px solid %2;border-radius:9px;"
                       "padding:8px 12px;}")
            .arg(kInk, kLine)));
    bubble->setMaximumWidth(UiZoom::s(520));
    addToStream(bubble, Qt::AlignLeft);
    runtime_->textBubbles.insert(partId, bubble);
    return bubble;
}

AgentChatPanel::ThinkingStrip* AgentChatPanel::thinkingStripFor(const QString& partId) {
    auto found = runtime_->thinkingStrips.constFind(partId);
    if (found != runtime_->thinkingStrips.constEnd()) return found.value();

    auto* strip = new ThinkingStrip;
    strip->setMaximumWidth(UiZoom::s(520));
    addToStream(strip, Qt::AlignLeft);
    runtime_->thinkingStrips.insert(partId, strip);
    return strip;
}
...
AgentChatPanel::ToolCard* AgentChatPanel::toolCardFor(const QString& partId) {
    ...
    card->setMaximumWidth(UiZoom::s(520));
    addToStream(card, Qt::AlignLeft);
    runtime_->toolCards.insert(partId, card);

    // 按钮直接连到控制器。permissionId 在按下的那一刻才去查——
    // 存一份在这儿的话，多端同时开着时别人先裁决了，这里存的就是个过期的 id。
    connect(card->allowButton(), &QPushButton::clicked, this, [this, partId] {
        replyForPart(partId, false, true);
    });
    connect(card->alwaysButton(), &QPushButton::clicked, this, [this, partId] {
        replyForPart(partId, true, true);
    });
    connect(card->denyButton(), &QPushButton::clicked, this, [this, partId] {
        replyForPart(partId, false, false);
    });
    return card;
}
    ...
    for (const MaiMessage& message :
        ...
        for (const MaiMessagePart& part : message.parts) {
            ...
            if (tool->state == MaiToolState::Completed && !tool->output.empty()) {
                card->setDetail(QStringLiteral("完成 · %1 字节")
                                    .arg(static_cast<int>(tool->output.size())));
...
void AgentChatPanel::refreshContextSize() {
    ...
    runtime_->contextSize->setText(
        characters == 0 ? QStringLiteral("还没开始")
                        : QStringLiteral("这次对话已经攒了约 %1 字").arg(characters));
}
