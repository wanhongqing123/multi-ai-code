...
QPainterPath cornerPath(const QRectF &rect, qreal radius, Corners corners) {
  ...
  if (top > 0) {
    path.arcTo(QRectF(rect.right() - top * 2, rect.top(), top * 2, top * 2), 90,
               -90);
  }
  path.lineTo(rect.right(), rect.bottom() - bottom);
  if (bottom > 0) {
    path.arcTo(QRectF(rect.right() - bottom * 2, rect.bottom() - bottom * 2,
                      bottom * 2, bottom * 2),
               0, -90);
  }
  ...
  if (bottom > 0) {
    path.arcTo(
        QRectF(rect.left(), rect.bottom() - bottom * 2, bottom * 2, bottom * 2),
        270, -90);
  }
  path.closeSubpath();
  return path;
...
public:
  ...
  void run(const QVector<MarkdownBlock> &blocks) {
    ...
    if (!out_.trailingNote.isEmpty() && !noteConsumed_) {
      ...
      bottom += addPlainText(out_.trailingNote.text, font,
                             out_.trailingNote.color, 0, bottom, width_)
                    .height;
    }
    out_.height = bottom;
  }
...
private:
  ...
  QFont codeFont() const { return monospace(theme_.codeBlockPixelSize); }
  ...
  RunMetrics addSpans(const QVector<MarkdownSpan> &spans, const QFont &base,
                      const QColor &color, qreal x, qreal y, qreal avail,
    ...
    for (const MarkdownSpan &span : spans) {
      ...
      if (span.styles.testFlag(MarkdownStyle::Bold))
        font.setWeight(QFont::Bold);
      if (span.styles.testFlag(MarkdownStyle::Italic))
        font.setItalic(true);
      if (span.styles.testFlag(MarkdownStyle::Strike))
      ...
      if (isLink)
        links.push_back({start, span.text.size(), span.href});
      if (isCode)
        codeRanges.push_back({start, span.text.size()});
    }
    ...
    if (text.isEmpty())
      return {};
    return addLaidOutText(text, base, formats, links, codeRanges, x, y, avail,
                          alignment);
  }
  ...
  RunMetrics addPlainText(const QString &text, const QFont &font,
                          const QColor &color, qreal x, qreal y, qreal avail,
                          Qt::Alignment alignment = Qt::AlignLeft,
  ...
  RunMetrics addLaidOutText(const QString &text, const QFont &base,
                            ...
                            const QVector<QPair<int, int>> &codeRanges, qreal x,
                            qreal y, qreal avail, Qt::Alignment alignment,
                            bool selectable = true) {
    TextRun run;
    ...
    const int pixelSize =
        base.pixelSize() > 0 ? base.pixelSize() : theme_.bodyPixelSize;
    qreal height = 0;
    qreal firstLineHeight = 0;
  ...
  qreal measureSpans(const QVector<MarkdownSpan> &spans,
  ...
  qreal layoutRange(const QVector<MarkdownBlock> &blocks, int from, int to,
                    int depth, qreal x, qreal avail, qreal y) {
    bool first = true;
    int previousBelow = 0;
    ...
    while (index < to) {
      ...
      if (block.quoteDepth() > depth) {
        ...
        if (!first)
          y += qMax(previousBelow, theme_.blockSpacing);
      ...
      if (!first)
        y += qMax(previousBelow, spacingAbove(block));
  ...
  qreal layoutQuote(const QVector<MarkdownBlock> &blocks, int from, int to,
                    int depth, qreal x, qreal avail, qreal y) {
    const MarkdownCalloutStyle style =
        theme_.calloutStyle(blocks[from].callout);
    ...
    if (!style.title.isEmpty()) {
      ...
      font.setWeight(QFont::Bold);
      inner += addPlainText(style.title, font, style.accent, innerX, inner,
                            innerAvail)
                   .height;
  ...
  qreal layoutBlock(const MarkdownBlock &block, qreal x, qreal avail, qreal y) {
    const QColor color =
        block.quoteDepth() > 0 ? theme_.quoteText : theme_.text;
    switch (block.kind) {
    case MarkdownBlockKind::Paragraph:
      return y + addSpans(block.spans, bodyFont(), color, x, y, avail,
                          Qt::AlignLeft, noteForBlock(block))
                     .height;
  ...
  qreal layoutCode(const MarkdownBlock &block, qreal x, qreal avail, qreal y) {
    ...
    const qreal emptyLineHeight =
        theme_.codeBlockPixelSize * theme_.lineHeightRatio;
    ...
    for (const QString &line : lines) {
      ...
      y += addPlainText(line, font, theme_.codeText, innerX, y, innerAvail)
               .height;
    }
    y += theme_.codePadding;
  ...
  QFont markerFont(bool bullet = false) const {
    QFont font = bodyFont();
    font.setPixelSize(bullet ? theme_.listBulletPixelSize
                             : theme_.listMarkerPixelSize);
    font.setWeight(QFont::DemiBold);
    return font;
  }
  ...
  qreal layoutList(const MarkdownBlock &block, const QColor &color, qreal x,
                   qreal avail, qreal y) {
    ...
    for (const MarkdownListItem &item : block.items) {
      if (item.hasCheckbox || item.number <= 0)
        continue;
      markerColumn =
          qMax(markerColumn, markerMetrics.horizontalAdvance(
    ...
    for (const MarkdownListItem &item : block.items) {
      ...
      const RunMetrics metrics =
          addSpans(item.spans, font, color, textX, y, textAvail);
      const qreal firstLine =
          metrics.firstLineHeight > 0
              ? metrics.firstLineHeight
      ...
      if (item.hasCheckbox) {
        ...
        addPlainText(marker, markerFont(bullet), theme_.listMarker, itemX,
                     top + markerBaselineOffset(bullet), markerColumn,
                     Qt::AlignRight, false);
      }
  ...
  qreal layoutTable(const MarkdownBlock &block, qreal x, qreal avail, qreal y) {
    ...
    for (const QVector<QVector<MarkdownSpan>> &row : block.rows) {
      for (int column = 0; column < row.size(); ++column) {
        natural[column] =
            qMax(natural[column], measureSpans(row[column], font));
      }
    }
    ...
    for (int column = 0; column < columns; ++column)
      total += natural[column] + 2 * padX;

    QVector<qreal> widths(columns, 0);
    const qreal minimum =
        qMin<qreal>(avail / columns, theme_.tablePixelSize * 4);
    if (total <= avail || total <= 0) {
      ...
      qreal scaled = 0;
      for (qreal value : widths)
        scaled += value;
      if (scaled > avail) {
        for (int column = 0; column < columns; ++column)
          widths[column] *= avail / scaled;
      }
    }
    ...
    for (int rowIndex = 0; rowIndex < block.rows.size(); ++rowIndex) {
      ...
      for (int column = 0; column < columns; ++column) {
        ...
        if (column < row.size()) {
          ...
          const RunMetrics metrics = addSpans(
              row[column], cellFont,
      ...
      if (rowHeight <= 0)
        rowHeight = theme_.tablePixelSize * theme_.lineHeightRatio;
...
void MarkdownLayout::setTrailingNote(const QString &text, const QColor &color,
                                     int pixelSize) {
...
void MarkdownLayout::layout(const MarkdownDocument &document,
                            const MarkdownTheme &theme, qreal width) {
  impl_->theme = theme;
  impl_->width = width;
  impl_->height = 0;
  impl_->runs.clear();
...
qreal MarkdownLayout::width() const { return impl_->width; }

qreal MarkdownLayout::height() const { return impl_->height; }
...
void MarkdownLayout::paint(QPainter *painter, const QPointF &origin,
  ...
  for (const Shape &shape : impl_->shapes) {
    ...
    if (clipped && !clip.intersects(rect))
      continue;
    painter->setPen(shape.stroke.isValid()
                        ? QPen(shape.stroke, shape.strokeWidth)
                        : QPen(Qt::NoPen));
    painter->setBrush(shape.fill.isValid() ? QBrush(shape.fill)
                                           : QBrush(Qt::NoBrush));
    if (shape.radius > 0) {
    ...
    QPainterPath path;
    path.moveTo(rect.left() + rect.width() * 0.24,
                rect.top() + rect.height() * 0.52);
    path.lineTo(rect.left() + rect.width() * 0.43,
                rect.top() + rect.height() * 0.72);
    path.lineTo(rect.left() + rect.width() * 0.78,
                rect.top() + rect.height() * 0.29);
    painter->setBrush(Qt::NoBrush);
    painter->setPen(QPen(Qt::white, qMax<qreal>(1.4, rect.width() / 8.0),
  ...
  for (const TextRun &run : impl_->runs) {
    ...
    if (clipped && !clip.intersects(bounds))
      continue;
...
QString MarkdownLayout::linkAt(const QPointF &point) const {
  for (const TextRun &run : impl_->runs) {
    ...
    for (int index = 0; index < run.layout->lineCount(); ++index) {
      ...
      if (local.x() > line.naturalTextWidth())
        return QString();
...
int MarkdownLayout::positionAt(const QPointF &point) const {
  ...
  for (const TextRun &run : impl_->runs) {
    ...
    const qreal distance =
        point.y() < top ? top - point.y() : point.y() - bottom;
  ...
  for (int index = 0; index < best->layout->lineCount(); ++index) {
    ...
    if (!last && local.y() > line.y() + line.height())
      continue;
...
int MarkdownLayout::textLength() const { return impl_->text.size(); }

const QString &MarkdownLayout::selectableText() const { return impl_->text; }
