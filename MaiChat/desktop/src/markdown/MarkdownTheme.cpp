...
QColor hex(const char *value) { return QColor(QString::fromLatin1(value)); }
...
QString monospaceFamily() { return QStringLiteral("JetBrains Mono"); }
...
MarkdownTheme MarkdownTheme::standard(qreal zoom) {
  ...
  theme.zoom = zoom;
  theme.bodyFamily = QStringLiteral("Inter");
  ...
  for (int level = 0; level < 6; ++level) {
    ...
    theme.headingSpacingAbove[level] =
        scale(theme.headingSpacingAbove[level], zoom);
    theme.headingSpacingBelow[level] =
        scale(theme.headingSpacingBelow[level], zoom);
  }
...
MarkdownCalloutStyle
MarkdownTheme::calloutStyle(MarkdownCallout callout) const {
  switch (callout) {
