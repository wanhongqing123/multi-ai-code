  ...
  QTimer::singleShot(100, [] {
    for (QWidget *widget : QApplication::topLevelWidgets()) {
      ...
      if (dialog == nullptr || !dialog->isVisible())
        continue;
      auto *confirm =
          dialog->findChild<QPushButton *>(QStringLiteral("appMessageDanger"));
      if (confirm == nullptr) {
        confirm = dialog->findChild<QPushButton *>(
...
void MainWindowLayoutTest::transientActivityBubbleIsReplacedByIncomingText() {
  ...
  emit fake->activityReceived(
      QStringLiteral("peer"),
      RemoteIMActivitySignal{QStringLiteral("machine:test"),
                             RemoteIMActivityKind::MachineTool, true, 12000,
                             1});
  QTRY_VERIFY(window.findChild<QWidget *>(
                  QStringLiteral("remoteImActivityRow")) != nullptr);
...
void MainWindowLayoutTest::transientActivityStaysAfterNewOutgoingMessage() {
  ...
  emit fake->activityReceived(
      QStringLiteral("peer"),
      RemoteIMActivitySignal{QStringLiteral("machine:test"),
                             RemoteIMActivityKind::MachineWorking, true, 12000,
                             1});
  auto *activityRow =
      window.findChild<QWidget *>(QStringLiteral("remoteImActivityRow"));
  QTRY_VERIFY(activityRow != nullptr);
  auto *activityBubble = activityRow->findChild<QWidget *>(
      QStringLiteral("remoteImActivityBubble"));
  ...
  auto *outgoingRow =
      window.findChild<QWidget *>(QStringLiteral("messageRowOutgoing"));
  QTRY_VERIFY(outgoingRow != nullptr);
  QLayout *messageLayout = activityRow->parentWidget()->layout();
  QVERIFY(messageLayout != nullptr);
  QVERIFY(messageLayout->indexOf(outgoingRow) <
          messageLayout->indexOf(activityRow));
...
static void dropOnComposer(QTextEdit *editor, QMimeData *mime) {
  ...
  QDragEnterEvent enter(pos.toPoint(), Qt::CopyAction, mime, Qt::LeftButton,
                        Qt::NoModifier);
  QApplication::sendEvent(target, &enter);
  QDragMoveEvent move(pos.toPoint(), Qt::CopyAction, mime, Qt::LeftButton,
                      Qt::NoModifier);
...
int highlightedRowCount(const QWidget &window) {
  ...
  for (QWidget *row : window.findChildren<QWidget *>()) {
    if (row->property("searchHit").toBool())
      ++highlighted;
  }
  return highlighted;
}
...
void MainWindowLayoutTest::selectedConversationRowReachesTheListEdge() {
  ...
  auto *list =
      window.findChild<QListWidget *>(QStringLiteral("conversationList"));
...
void MainWindowLayoutTest::
    selectedConversationRowReachesTheListEdgeWithScrollBar() {
  auto client = std::make_unique<FakeRemoteIMClient>();
  RemoteIMApplication app(QStringLiteral("desktop-user"), std::move(client));
  for (int i = 0; i < 40; ++i) {
  ...
  QVERIFY2(gap <= tolerance,
           qPrintable(
               QStringLiteral("有滚动条时选中行右侧仍留白 %1 物理像素（容 %2；"
...
bool dragScrollBarHandle(QScrollBar *bar, int dyLogical) {
  ...
  const QRect handle = bar->style()->subControlRect(
  ...
  QMouseEvent move(QEvent::MouseMove, to, bar->mapToGlobal(to), Qt::NoButton,
...
void MainWindowLayoutTest::
    listScrollBarsAreNarrowAndStillScrollableAcrossZoomLevels() {
  struct RestoreZoom {
    qreal old;
    ~RestoreZoom() { UiZoom::setFactor(old); }
  } restore{UiZoom::factor()};
  ...
  for (const qreal zoom : {0.8, 1.0, 1.5, 2.0}) {
    ...
    QVERIFY2(
        bar->isVisible(),
        qPrintable(
            QStringLiteral("缩放 %1：40 条会话却没有纵向滚动条").arg(zoom)));
    ...
    QVERIFY2(
        ...
        qPrintable(
            QStringLiteral("缩放 %1：滚动范围为空，无法验证滚动").arg(zoom)));
    bar->setValue(bar->maximum());
    QCOMPARE(bar->value(), bar->maximum());
    bar->setValue(bar->minimum());
    QCOMPARE(bar->value(), bar->minimum());
    QWheelEvent wheel(
        QPointF(list->viewport()->rect().center()),
    ...
    QCoreApplication::processEvents();
    QVERIFY2(
        dragScrollBarHandle(bar, 120),
        qPrintable(QStringLiteral("缩放 %1：会话列表滑块矩形为空，无法拖动")
                       .arg(zoom)));
    QVERIFY2(
        bar->value() > bar->minimum(),
        qPrintable(QStringLiteral(
                       "缩放 %1：按住会话列表滑块拖动后 value 没变（仍为 %2）")
                       .arg(zoom)
                       .arg(bar->value())));

    // 联系人列表用的是同一套样式，一并验证它也拖得动。
    auto *contacts =
        window.findChild<QListWidget *>(QStringLiteral("contactsList"));
    QVERIFY(contacts != nullptr);
    auto *contactsNav =
        window.findChild<QPushButton *>(QStringLiteral("contactsNavButton"));
    ...
    QVERIFY2(qAbs(contactsBar->width() - expected) <= 1,
             qPrintable(
                 QStringLiteral("缩放 %1：联系人列表滚动条宽 %2px，期望约 %3px")
                     .arg(zoom)
                     .arg(contactsBar->width())
                     .arg(expected)));
    contactsBar->setValue(contactsBar->minimum());
    QCoreApplication::processEvents();
    QVERIFY2(dragScrollBarHandle(contactsBar, 120),
             qPrintable(
                 QStringLiteral("缩放 %1：联系人列表滑块矩形为空").arg(zoom)));
    QVERIFY2(
        contactsBar->value() > contactsBar->minimum(),
        qPrintable(QStringLiteral("缩放 %1：拖动联系人列表滑块后 value 没变")
                       .arg(zoom)));
  }
...
void MainWindowLayoutTest::conversationListPutsNewestMessageFirst() {
  ...
  for (const Seed &seed : seeds) {
    RemoteIMMessage m;
    QVERIFY(
        app.chatState().latestMessageWith(QString::fromLatin1(seed.peer), &m));
  ...
  QVERIFY2(order.size() >= 4,
           qPrintable(QStringLiteral("会话数不对：%1").arg(order.size())));
...
void MainWindowLayoutTest::conversationListBreaksTimeTiesByName() {
  ...
  QCOMPARE(order, QStringList({QStringLiteral("alpha"), QStringLiteral("bravo"),
...
void MainWindowLayoutTest::replyBarShowsTargetAndClearsAfterSending() {
  ...
  auto *replyBar =
      window.findChild<QWidget *>(QStringLiteral("pendingReplyBar"));
  auto *replyLabel =
      window.findChild<QLabel *>(QStringLiteral("pendingReplyLabel"));
  ...
  QVERIFY2(replyLabel->text().contains(QStringLiteral("原始消息内容")),
           qPrintable(QStringLiteral("提示条没显示回复对象：") +
                      replyLabel->text()));

  editor->setPlainText(QStringLiteral("这是回复内容"));
  // 点真实的发送按钮，走用户实际走的那条路。
  auto *sendButton =
      window.findChild<QPushButton *>(QStringLiteral("sendButton"));
  QVERIFY(sendButton != nullptr);
  QTest::mouseClick(sendButton, Qt::LeftButton);
  QCoreApplication::processEvents();

  QVERIFY2(replyBar->isHidden(),
           "发送后提示条必须收起，否则下一条会带上旧引用");
...
void MainWindowLayoutTest::clickingQuoteBlockJumpsToTheOriginalMessage() {
  ...
  QCoreApplication::processEvents();
  const QList<RemoteIMMessage> seeded =
      app.chatState().messagesWith(QStringLiteral("phone-user"));
  ...
  QVERIFY2(!quote.msgId.isEmpty(),
           "引用必须带上原始 SDK 消息 ID，否则块根本不可点");
  ...
  const auto highlightedRow = [&window]() -> QWidget * {
    ...
    for (QWidget *candidate : window.findChildren<QWidget *>()) {
      if (!candidate->property("searchHit").toBool())
        continue;
  ...
  // 只断言存在高亮的话，跳到任意一条都能过。
  auto *body =
      row->findChild<MarkdownLabel *>(QStringLiteral("messageMarkdownView"));
  QVERIFY2(body != nullptr, "高亮的行里应当有正文");
  QVERIFY2(
      body->plainText().contains(QStringLiteral("这是被引用的原始消息")),
...
void MainWindowLayoutTest::clickingQuoteBlockSaysSoWhenTheOriginalIsNotLocal() {
  ...
  QVERIFY2(toast != nullptr && toast->isVisible(),
           "定位不到原消息时必须给出提示");
...
void MainWindowLayoutTest::quotedReplyRendersQuoteBlockAboveBody() {
  ...
  QCoreApplication::processEvents();
  // 没有引用时不该凭空冒出引用块。
  QVERIFY(window.findChild<QWidget *>(QStringLiteral("messageQuoteBlock")) ==
          nullptr);
  ...
  QWidget *block =
      window.findChild<QWidget *>(QStringLiteral("messageQuoteBlock"));
  QVERIFY2(block != nullptr, "带引用的消息必须渲染出引用块");
  ...
  const auto currentQuoteText = [&window] {
    const auto *label =
        window.findChild<QLabel *>(QStringLiteral("messageQuoteText"));
    return label ? label->text() : QString();
  };
  QTRY_VERIFY2(
      currentQuoteText().contains(QStringLiteral("这是被引用的原始消息")),
      qPrintable(QStringLiteral("引用块内容不对：") + currentQuoteText()));
  QVERIFY(currentQuoteText().contains(QStringLiteral("phone-user")));
}

void MainWindowLayoutTest::
    droppingFilesIntoComposerAttachesThemInsteadOfPastingPaths() {
  QTemporaryDir dir;
  QVERIFY(dir.isValid());
  const QString docPath =
      QDir(dir.path()).filePath(QStringLiteral("report.txt"));
  QFile doc(docPath);
...
void MainWindowLayoutTest::droppingAnImageFileSendsTheOriginalFileAsAnImage() {
  ...
  const QString imagePath =
      QDir(dir.path()).filePath(QStringLiteral("photo.png"));
...
void MainWindowLayoutTest::settingsPanelBorderIsNotCoveredByRows() {
  ...
  const auto describeMiss = [](int y, QRgb first, QRgb second) {
    return QStringLiteral("y=%1 [%2,%3]")
        .arg(y)
        .arg(QString::number(first & 0x00ffffff, 16)
                 .rightJustified(6, QLatin1Char('0')))
        .arg(QString::number(second & 0x00ffffff, 16)
                 .rightJustified(6, QLatin1Char('0')));
  };
  // 圆角边界会产生平台相关的抗锯齿像素，竖边检查应避开四个圆角。
  const int cornerGuard = UiZoom::s(12);
  for (int y = cornerGuard; y < painted.height() - cornerGuard;
       y += UiZoom::s(12)) {
    ++sampled;
  ...
  QVERIFY2(
      leftHits == sampled,
      qPrintable(QStringLiteral(
                     "左边框只在 %1/%2 个采样点上存在，面板 %3x%4，缺失：%5")
                     .arg(leftHits)
                     .arg(sampled)
                     .arg(painted.width())
                     .arg(painted.height())
  ...
  QVERIFY2(
      rightHits == sampled,
      qPrintable(QStringLiteral(
                     "右边框只在 %1/%2 个采样点上存在，面板 %3x%4，缺失：%5")
                     .arg(rightHits)
                     .arg(sampled)
                     .arg(painted.width())
                     .arg(painted.height())
...
void MainWindowLayoutTest::exposesDesktopChatLayoutControls() {
  ...
  QVERIFY(window.findChild<QWidget *>(QStringLiteral("conversationPane")) !=
          nullptr);
  QVERIFY(window.findChild<QWidget *>(QStringLiteral("chatContentPane")) !=
          nullptr);
  QVERIFY(window.findChild<QTextEdit *>(QStringLiteral("messageEditor")) !=
          nullptr);
  QVERIFY(window.findChild<QPushButton *>(QStringLiteral("sendButton")) !=
          nullptr);
}
...
void MainWindowLayoutTest::composerUsesEmbeddedIconSendAction() {
  ...
  const int bottomInset =
      editor->height() - sendButton->geometry().bottom() - 1;
...
void MainWindowLayoutTest::conversationPreviewUsesPlainMarkdown_data() {
  ...
  QTest::newRow("heading-inline")
      << QStringLiteral("## 标题\n\n**重点** 与 `code`")
      << QStringLiteral("标题 重点 与 code") << false;
  QTest::newRow("link-strike")
      << QStringLiteral("[文档](https://example.com) 与 ~~旧版~~")
      << QStringLiteral("文档 与 旧版") << false;
  QTest::newRow("code-literals")
      << QStringLiteral("```cpp\na*b*c\npath_with_under_score\n```")
      << QStringLiteral("a*b*c path_with_under_score") << false;
  QTest::newRow("callout") << QStringLiteral("> [!TIP]\n> 使用 **独立目录**")
                           << QStringLiteral("建议：使用 独立目录") << false;
  QTest::newRow("same-line-preview-callout")
      << QStringLiteral("> [!TIP] 正文") << QStringLiteral("建议：正文")
      << false;
  QTest::newRow("emphasized-preview-callout")
      << QStringLiteral("> **[!TIP]** 正文") << QStringLiteral("建议：正文")
      << false;
  QTest::newRow("linked-literal-callout")
      << QStringLiteral("> [[!TIP]](https://example.com)")
      << QStringLiteral("[!TIP]") << false;
  QTest::newRow("tasks") << QStringLiteral("- [x] 完成\n- [ ] 待办")
                         << QStringLiteral("☑ 完成 ☐ 待办") << false;
  QTest::newRow("table") << QStringLiteral("| A | B |\n|---|---|\n|1|2|")
                         << QStringLiteral("A B 1 2") << false;
  QTest::newRow("reference-link")
      << QStringLiteral("[文档][ref]\n\n[ref]: https://example.com")
      << QStringLiteral("文档") << false;
  QTest::newRow("image-alt")
      << QStringLiteral("![替代文字](https://example.com/a.png)")
      << QStringLiteral("替代文字") << false;
  QTest::newRow("inline-code-literals")
      << QStringLiteral("`**literal**` 与 `a_b`")
      << QStringLiteral("**literal** 与 a_b") << false;
  QTest::newRow("callout-in-code") << QStringLiteral("```text\n> [!TIP]\n```")
                                   << QStringLiteral("> [!TIP]") << false;
  QTest::newRow("inline-callout-code")
      << QStringLiteral("> `[!TIP]`") << QStringLiteral("[!TIP]") << false;
  QTest::newRow("crlf") << QStringLiteral("## 标题\r\n\r\n下一段")
                        << QStringLiteral("标题 下一段") << false;
  QTest::newRow("attachment")
      << QStringLiteral("[文件消息] **report**.md")
      << QStringLiteral("[文件消息] **report**.md") << true;
  QTest::newRow("only-markup")
      << QStringLiteral("---") << QStringLiteral("新消息") << false;
  const QString emoji = QString::fromUtf8("👨‍👩‍👧‍👦");
  QTest::newRow("long-emoji")
      << emoji.repeated(170) << (emoji.repeated(160) + QStringLiteral("…"))
      << false;
  for (const QString &sequence :
       {emoji, QString::fromUtf8("👩🏽‍💻"), QString::fromUtf8("❤️"),
        QString::fromUtf8("1️⃣"), QString::fromUtf8("🇨🇳")}) {
    const QByteArray name =
        QByteArray("emoji-after-159-") + sequence.toUtf8().toHex();
    QTest::newRow(name.constData())
        << (QString(159, QLatin1Char('a')) + sequence + sequence)
        << (QString(159, QLatin1Char('a')) + sequence + QStringLiteral("…"))
        << false;
  }
}
...
void MainWindowLayoutTest::conversationPreviewUsesPlainMarkdown() {
  ...
  QVERIFY(
      app.chatState().latestMessageWith(QStringLiteral("phone-user"), &stored));
...
void MainWindowLayoutTest::compactSplittersKeepDragTargets() {
  ...
  const int gap = horizontal ? second.left() - first.right() - 1
                             : second.top() - first.bottom() - 1;
  QVERIFY2(gap <= UiZoom::s(1),
           qPrintable(QStringLiteral("visible splitter gap=%1").arg(gap)));
  QVERIFY(gap > 0);
  ...
  QMouseEvent move(QEvent::MouseMove, QPointF(finish),
                   QPointF(handle->mapToGlobal(finish)), Qt::NoButton,
                   Qt::LeftButton, Qt::NoModifier);
  QApplication::sendEvent(handle, &move);
  QTest::mouseRelease(handle, Qt::LeftButton, Qt::NoModifier, finish);
  QVERIFY2(splitter->sizes().at(0) != before,
           "The thin splitter must remain draggable");
}
...
void MainWindowLayoutTest::exposesResizableSplitters() {
  ...
  auto *messageComposerSplitter =
      window.findChild<QSplitter *>(QStringLiteral("messageComposerSplitter"));
...
void MainWindowLayoutTest::rendersEmptyConversationState() {
  ...
  QVERIFY(window.findChild<QWidget *>(QStringLiteral("emptyMessagesView")) !=
          nullptr);
}
...
void MainWindowLayoutTest::sendsMultilineTextWithoutFlatteningReturns() {
  ...
  auto *markdownView =
      window.findChild<MarkdownLabel *>(QStringLiteral("messageMarkdownView"));
...
void MainWindowLayoutTest::rendersSentMessageFromTopWithMetadata() {
  ...
  auto *sendButton =
      window.findChild<QPushButton *>(QStringLiteral("sendButton"));
  auto *messageLayout =
      window.findChild<QVBoxLayout *>(QStringLiteral("messageLayout"));
  ...
  auto *loadEarlierButton =
      window.findChild<QPushButton *>(QStringLiteral("loadEarlierButton"));
  QVERIFY(loadEarlierButton != nullptr);
  QVERIFY(loadEarlierButton->isHidden());
  QVERIFY(messageLayout->itemAt(1)->widget() != nullptr);
  QCOMPARE(messageLayout->itemAt(1)->widget()->objectName(),
           QStringLiteral("messageRowOutgoing"));
  QVERIFY(window.findChild<QWidget *>(
              QStringLiteral("messageBubbleOutgoing")) != nullptr);

  QVERIFY(window.findChild<QLabel *>(QStringLiteral("messageAuthorLabel")) ==
          nullptr);
  QVERIFY(window.findChild<QLabel *>(QStringLiteral("messageTimeLabel")) ==
          nullptr);
  QVERIFY(window.findChild<QLabel *>(QStringLiteral("messageAvatarOutgoing")) ==
          nullptr);
  auto *markdown =
      window.findChild<MarkdownLabel *>(QStringLiteral("messageMarkdownView"));
  ...
  QVERIFY(
      markdown->plainText().startsWith(QStringLiteral("hello from desktop")));
  ...
  QVERIFY2(
      !markdown->plainText().contains(QLatin1Char('\n')),
      qPrintable(QStringLiteral("时间戳另起了一行：") + markdown->plainText()));
  auto *statusLabel =
      window.findChild<QLabel *>(QStringLiteral("messageStatusLabel"));
  ...
  QVERIFY(statusLabel->styleSheet().contains(
      QStringLiteral("border: 1px solid #12a150")));
  QVERIFY(statusLabel->styleSheet().contains(
      QStringLiteral("background: transparent")));
  // 状态图标要和气泡平级挂在消息行上，不能落进气泡内部。
  // 必须从 statusLabel 反查所在行，不能用 window.findChild
  // 直接找气泡：pending→sent 是「建新行 + 旧行 deleteLater + 插入新行」，旧
  // pending 行在事件循环真正销毁前仍留在 对象树里，findChild
  // 可能拿到那条旧气泡。而 Pending 态不生成状态图标，statusLabel
  ...
  QVERIFY(
      statusRow->findChild<QWidget *>(QStringLiteral("messageBubbleOutgoing"),
...
void MainWindowLayoutTest::rendersRelativeMessageDates() {
  ...
  for (const auto *view : window.findChildren<MarkdownLabel *>(
           QStringLiteral("messageMarkdownView"))) {
  ...
  QVERIFY(actualTimes.contains(
      olderTime.toString(QStringLiteral("M 月 d 日 HH:mm"))));
}
...
void MainWindowLayoutTest::contactsNavigationShowsContactsAndOpensChat() {
  ...
  auto *contactsPage =
      window.findChild<QWidget *>(QStringLiteral("contactsPage"));
  auto *contentStack =
  ...
  QCOMPARE(contactsList->item(0)->data(Qt::UserRole).toString(),
           QStringLiteral("phone-user"));

  const QRect itemRect = contactsList->visualItemRect(contactsList->item(0));
  QTest::mouseClick(contactsList->viewport(), Qt::LeftButton, Qt::NoModifier,
                    itemRect.center());
  QCOMPARE(app.chatState().selectedPeerId(), QStringLiteral("phone-user"));
  auto *messagesPage =
      window.findChild<QWidget *>(QStringLiteral("messagesPage"));
...
void MainWindowLayoutTest::settingsNavigationShowsAccountAndSdkDefaults() {
  ...
  auto *settingsPage =
      window.findChild<QWidget *>(QStringLiteral("settingsPage"));
  auto *contentStack =
      window.findChild<QStackedWidget *>(QStringLiteral("contentStack"));
  auto *accountValue =
      window.findChild<QLabel *>(QStringLiteral("settingsAccountValue"));
  auto *connectionValue =
      window.findChild<QLabel *>(QStringLiteral("settingsConnectionValue"));
  auto *sdkAppIdValue =
      window.findChild<QLabel *>(QStringLiteral("settingsSdkAppIdValue"));
...
void MainWindowLayoutTest::agentSettingsDialogExposesSecureModelFields() {
  ...
  QTimer::singleShot(100, [&] {
    auto *dialog =
        window.findChild<QDialog *>(QStringLiteral("agentModelDialog"));
    if (dialog == nullptr)
      return;
    sawDialog = true;
    auto *baseUrl =
    ...
    auto *model =
        dialog->findChild<QLineEdit *>(QStringLiteral("agentModelName"));
    auto *apiKey =
...
void MainWindowLayoutTest::agentSettingsSaveAppliesWithoutRestart() {
  ...
  QTimer::singleShot(100, [&] {
    ...
    auto *apiKey =
        dialog->findChild<QLineEdit *>(QStringLiteral("agentModelApiKey"));
    auto *save =
        dialog->findChild<QPushButton *>(QStringLiteral("agentModelSave"));
    if (baseUrl == nullptr || model == nullptr || apiKey == nullptr ||
        save == nullptr)
      return;
...
void MainWindowLayoutTest::leftNavigationRailIsResizableAndWider() {
  ...
  auto *rootNavigationSplitter =
      window.findChild<QSplitter *>(QStringLiteral("rootNavigationSplitter"));
  ...
  // 导航按钮按需求只留图标：文字移到 accessibleName /
  // tooltip。这里同时钉住三点——
  ...
  QVERIFY(QTest::qWaitForWindowExposed(&window));
  for (QPushButton *button : navButtons) {
    QVERIFY2(button->iconSize().width() >= 24,
             "导航栏图标应当比会话栏的「+」大一档");
...
void MainWindowLayoutTest::removesRedundantChromeLabels() {
  ...
  QVERIFY(window.findChild<QLabel *>(QStringLiteral("accountLabel")) ==
          nullptr);
}

void MainWindowLayoutTest::
    conversationListsUseDelegateItemsForSmoothScrolling() {
  auto client = std::make_unique<FakeRemoteIMClient>();
  RemoteIMApplication app(QStringLiteral("desktop-user"), std::move(client));
  for (int index = 0; index < 80; ++index) {
    app.addContact(QStringLiteral("phone-user-%1").arg(index),
                   QStringLiteral("iPhone %1").arg(index));
  }
  app.chatState().upsertContact(RemoteIMContact{
      QStringLiteral("phone-user-0"), QStringLiteral("iPhone 0"),
      QStringLiteral("https://example.com/iphone.png")});

  MainWindow window(app);
  auto *conversationList =
      window.findChild<QListWidget *>(QStringLiteral("conversationList"));
  auto *contactsNavButton =
      window.findChild<QPushButton *>(QStringLiteral("contactsNavButton"));
  ...
  for (int index = 0; index < conversationList->count(); ++index) {
    QVERIFY(conversationList->itemWidget(conversationList->item(index)) ==
            nullptr);
  }
...
void MainWindowLayoutTest::rendersMarkdownMessageContent() {
  ...
  const auto blocks =
      MarkdownDocument::parse(markdownView->markdown()).blocks();
  ...
  for (const MarkdownListItem &item : blocks.at(2).items) {
    for (const MarkdownSpan &span : item.spans) {
      if (span.href == QStringLiteral("https://example.com"))
        sawLink = true;
    }
  }
  QVERIFY(sawLink);
...
void MainWindowLayoutTest::notificationClickPreservesWindowState_data() {
  ...
  QTest::newRow("maximized-minimized")
      << int(Qt::WindowMaximized | Qt::WindowMinimized);
...
void MainWindowLayoutTest::markdownBubbleHasCompactBottomSpacing() {
  ...
  auto *view =
      window.findChild<MarkdownLabel *>(QStringLiteral("messageMarkdownView"));
  auto *bubble =
      window.findChild<QWidget *>(QStringLiteral("messageBubbleIncoming"));
  ...
  const int bottomInset =
      bubble->height() - (view->mapTo(bubble, QPoint()).y() + view->height());
  QVERIFY2(
      bottomInset >= 0 && bottomInset <= UiZoom::s(8) + 1,
...
void MainWindowLayoutTest::rendersApprovalButtonsAndSendsStructuredDecision() {
  ...
  auto *fakeClient = client.get();
  RemoteIMApplication app(QStringLiteral("desktop-user"), std::move(client));
  app.addContact(QStringLiteral("multi-ai-code"),
  ...
  request.approvalRequest = RemoteIMApprovalRequest{
      ...
      {RemoteIMApprovalAction::ApproveOnce,
       RemoteIMApprovalAction::ApprovePrefix, RemoteIMApprovalAction::Reject}};
  ...
  auto *prefixButton = window.findChild<QPushButton *>(
      QStringLiteral("approvalApprovePrefixButton"));
  QVERIFY(prefixButton != nullptr);
  QVERIFY(window.findChild<QPushButton *>(
              QStringLiteral("approvalRejectButton")) != nullptr);
  ...
  auto *sendingLabel =
      window.findChild<QLabel *>(QStringLiteral("approvalSentLabel"));
  ...
  prefixButton = window.findChild<QPushButton *>(
      QStringLiteral("approvalApprovePrefixButton"));
  QVERIFY(prefixButton != nullptr);
  QVERIFY(prefixButton->isVisible());
  QVERIFY(window.findChild<QLabel *>(QStringLiteral("approvalSentLabel")) ==
          nullptr);
  ...
  QCOMPARE(fakeClient->lastApprovalAction(),
...
void MainWindowLayoutTest::restoresSubmittedApprovalStateFromDatabase() {
  ...
  const QString databasePath = dir.filePath(QStringLiteral("messages.db"));
  {
    ...
    database.upsertContact(RemoteIMContact{QStringLiteral("multi-ai-code"),
                                           QStringLiteral("Multi-AI Code")});
    ...
    QVERIFY(database.insertMessageIfAbsent(request,
                                           QStringLiteral("multi-ai-code")));
    ...
    decision.approvalDecision =
        RemoteIMApprovalDecision{QStringLiteral("approval-restart-1"),
                                 RemoteIMApprovalAction::ApproveOnce};
    QVERIFY(database.insertMessageIfAbsent(decision,
                                           QStringLiteral("multi-ai-code")));
  }
...
void MainWindowLayoutTest::
    authoritativeApprovalResolutionOverridesLateSentDecision() {
  auto client = std::make_unique<FakeRemoteIMClient>();
  RemoteIMApplication app(QStringLiteral("desktop-user"), std::move(client));
  app.addContact(QStringLiteral("multi-ai-code"),
                 QStringLiteral("Multi-AI Code"));
  ...
  resolution.approvalDecision =
      RemoteIMApprovalDecision{QStringLiteral("approval-authoritative-1"),
...
void MainWindowLayoutTest::copiesOriginalMarkdownFromMessageContextMenu() {
  ...
  auto *copyOriginalAction =
      window.findChild<QAction *>(QStringLiteral("copyOriginalDataAction"));
  QVERIFY(markdownView != nullptr);
...
void MainWindowLayoutTest::addContactButtonSitsBesideTheSearchBox() {
  ...
  MainWindow window(app);
  auto *navRail = window.findChild<QWidget *>(QStringLiteral("navRail"));
  const QList<QPushButton *> addButtons = window.findChildren<QPushButton *>(
      QStringLiteral("addConversationButton"));
...
void MainWindowLayoutTest::sectionTitleFollowsSelectedNavigation() {
  ...
  app.addContact(QStringLiteral("phone-user"), QStringLiteral("iPhone"));
  app.chatState().receiveText(QStringLiteral("phone-user"),
                              QStringLiteral("通讯录里不应该显示这条消息摘要"));
  ...
  auto *messagesTitle =
      window.findChild<QLabel *>(QStringLiteral("messagesSectionTitle"));
  auto *contactsTitle =
      window.findChild<QLabel *>(QStringLiteral("contactsSectionTitle"));
  auto *contactsNavButton =
      window.findChild<QPushButton *>(QStringLiteral("contactsNavButton"));
  auto *contentStack =
      window.findChild<QStackedWidget *>(QStringLiteral("contentStack"));
  ...
  for (QLabel *label : labels) {
    QVERIFY2(label->text() != QStringLiteral("通讯录"),
             "不该再有「通讯录」这行标题文字");
  }
  auto *search =
      window.findChild<QLineEdit *>(QStringLiteral("globalSearchBox"));
  QVERIFY2(search != nullptr &&
               search->parentWidget() == conversationList->parentWidget(),
           "搜索框应当就在会话列表这一列的头部");
  auto *contactsSearch =
      window.findChild<QLineEdit *>(QStringLiteral("contactsSearchBox"));
  ...
  QVERIFY(conversationList->item(0)
              ->data(Qt::UserRole + 2)
              .toString()
              .contains(QStringLiteral("通讯录里不应该显示")));
}
...
void MainWindowLayoutTest::contactSearchFiltersDirectoryFuzzily() {
  ...
  auto visibleNames = [contactsList] {
    ...
    for (int row = 0; row < contactsList->count(); ++row) {
      if (contactsList->item(row)->isHidden())
        continue;
...
void MainWindowLayoutTest::contactsDirectoryUsesSingleLineRows() {
  auto client = std::make_unique<FakeRemoteIMClient>();
  RemoteIMApplication app(QStringLiteral("desktop-user"), std::move(client));
  app.addContact(QStringLiteral("house-obs-studio"),
                 QStringLiteral("house-obs-studio"));
...
void MainWindowLayoutTest::receivedMessagesKeepTheSameLeftEdge() {
  ...
  window.show();
  QVERIFY(QTest::qWaitForWindowExposed(&window));
  QVERIFY(window.findChild<QLabel *>(QStringLiteral("messageAvatarIncoming")) ==
          nullptr);
  auto *view =
      window.findChild<MarkdownLabel *>(QStringLiteral("messageMarkdownView"));
...
void MainWindowLayoutTest::wideChatUsesWiderMessageBubbles() {
  ...
  fakeClient->emitIncomingText(
      QStringLiteral("phone-user"),
      QStringLiteral(
          "这是一段比较长的 AICLI "
          "输出内容，用来验证桌面宽屏下消息气泡不会过窄，"
  ...
  QVERIFY(window.findChild<QLabel *>(QStringLiteral("messageAuthorLabel")) ==
          nullptr);
  QVERIFY(window.findChild<QLabel *>(QStringLiteral("messageRelationBadge")) ==
          nullptr);
  ...
  QTRY_VERIFY(incomingBubble->width() >=
              incomingRow->contentsRect().width() - 2);
  auto *outgoingRow =
      window.findChild<QWidget *>(QStringLiteral("messageRowOutgoing"));
  QVERIFY(outgoingRow);
  auto *outgoingView = outgoingRow->findChild<MarkdownLabel *>(
      QStringLiteral("messageMarkdownView"));
  QVERIFY(outgoingView);
  QVERIFY(outgoingView->plainText().endsWith(
      outgoingView->property("messageTimestamp").toString()));
}
...
void MainWindowLayoutTest::restoredLongMessagesExpandAfterWindowIsShown() {
  ...
  RemoteIMApplication app(QStringLiteral("desktop-user"), std::move(client));
  app.addContact(QStringLiteral("phone-user"), QStringLiteral("iPhone"));
  app.chatState().receiveText(
      QStringLiteral("phone-user"),
      QStringLiteral("这是一段在窗口显示前就已经存在的历史长消息，用来模拟登录"
                     "后拉取到的 AICLI 输出。"
                     "窗口完成布局后，这类历史消息也应该使用宽屏消息区的可用宽"
                     "度，不能继续保持窄气泡。"));
  ...
  QVERIFY(incomingBubble != nullptr);
  auto *messageScroll =
      window.findChild<QScrollArea *>(QStringLiteral("messageScroll"));
  QVERIFY(messageScroll != nullptr);
  QTRY_VERIFY2(
      incomingBubble->minimumWidth() >= 820,
      qPrintable(
          QStringLiteral("min=%1 max=%2 viewport=%3 expanded=%4")
...
void MainWindowLayoutTest::slashCommandSuggestionsFillComposer() {
  ...
  auto *editor = window.findChild<QTextEdit *>(QStringLiteral("messageEditor"));
  auto *commandBar =
      window.findChild<QWidget *>(QStringLiteral("slashCommandBar"));
  ...
  // 悬浮层，会吞掉紧随其后的 KeyRelease，令 Windows
  // 认为按键仍按住而狂发自动重复
  // （输入 /g 变成一长串 g）。因此每次改动输入框后放行一次事件循环，等 0ms
  // 单次定时器
  // 触发、命令栏完成重建，再做同步断言。
  ...
  for (const QString &objectName : expectedCommandObjectNames) {
    QVERIFY2(window.findChild<QPushButton *>(objectName) != nullptr,
             qPrintable(objectName));
  }

  auto *statusButton = window.findChild<QPushButton *>(
      QStringLiteral("slashCommandButton_status"));
  ...
  auto *planButton = window.findChild<QPushButton *>(
      QStringLiteral("slashCommandButton_plan"));
  ...
  auto *buildButton = window.findChild<QPushButton *>(
      QStringLiteral("slashCommandButton_build"));
  ...
  auto *modelsButton = window.findChild<QPushButton *>(
      QStringLiteral("slashCommandButton_models"));
  ...
  auto *modelButton = window.findChild<QPushButton *>(
      QStringLiteral("slashCommandButton_model"));
  ...
  auto *goalButton = window.findChild<QPushButton *>(
      QStringLiteral("slashCommandButton_goal"));
  ...
  auto *btwButton =
      window.findChild<QPushButton *>(QStringLiteral("slashCommandButton_btw"));
  ...
  auto *diffButton = window.findChild<QPushButton *>(
      QStringLiteral("slashCommandButton_diff"));
  ...
  auto *interruptButton = window.findChild<QPushButton *>(
      QStringLiteral("slashCommandButton_interrupt"));
  ...
  auto *compactButton = window.findChild<QPushButton *>(
      QStringLiteral("slashCommandButton_compact"));
  ...
  auto *helpButton = window.findChild<QPushButton *>(
      QStringLiteral("slashCommandButton_help"));
...
void MainWindowLayoutTest::slashCommandBarLeavesImeCompositionUndisturbed() {
  ...
  // 输入 "/goal " 让命令栏显示（含 /goal 按钮）。setPlainText
  // 会把光标留在开头，
  ...
  QVERIFY(window.findChild<QPushButton *>(
              QStringLiteral("slashCommandButton_goal")) != nullptr);

  // 模拟输入法开始组词（预编辑串
  // "n"，未上屏）。命令栏不得在组词期间被重建/隐藏，
  ...
  // 组词上屏 "你好"：组词结束后命令栏才刷新，"/goal 你好" 不匹配任何命令 →
  // 隐藏。
  {
...
void MainWindowLayoutTest::
    deleteKeyClearsMessagesButKeepsContactInConversationList() {
  auto client = std::make_unique<FakeRemoteIMClient>();
  RemoteIMApplication app(QStringLiteral("desktop-user"), std::move(client));
  app.addContact(QStringLiteral("phone-user"), QStringLiteral("iPhone"));
  app.chatState().receiveText(QStringLiteral("phone-user"),
                              QStringLiteral("remove me"));
  app.addContact(QStringLiteral("other-user"), QStringLiteral("Other"));
  ...
  QCOMPARE(conversationList->currentItem()->data(Qt::UserRole).toString(),
           QStringLiteral("phone-user"));
  confirmNextContactDeletion();
  QTest::keyClick(conversationList, Qt::Key_Delete);

  // 会话列表只清空聊天记录：好友保留（删除好友是通讯录的功能）。
  QCOMPARE(app.chatState().messagesWith(QStringLiteral("phone-user")).size(),
           0);
  QCOMPARE(app.chatState().contacts().size(), 2);
  QCOMPARE(app.chatState().contacts().first().userId,
           QStringLiteral("phone-user"));
}

void MainWindowLayoutTest::
    deleteKeyRemovesContactAndMessagesFromContactsList() {
  auto client = std::make_unique<FakeRemoteIMClient>();
  RemoteIMApplication app(QStringLiteral("desktop-user"), std::move(client));
  app.addContact(QStringLiteral("phone-user"), QStringLiteral("iPhone"));
  app.chatState().receiveText(QStringLiteral("phone-user"),
  ...
  QCOMPARE(contactsList->currentItem()->data(Qt::UserRole).toString(),
           QStringLiteral("phone-user"));
  confirmNextContactDeletion();
...
void MainWindowLayoutTest::conversationListShowsUnreadBadgeAndClearsOnOpen() {
  ...
  app.chatState().receiveText(QStringLiteral("phone-user"),
                              QStringLiteral("已读消息"));
  app.chatState().receiveText(QStringLiteral("mac-user"),
                              QStringLiteral("未读一"));
  app.chatState().receiveText(QStringLiteral("mac-user"),
                              QStringLiteral("未读二"));
  ...
  for (int row = 0; row < conversationList->count(); ++row) {
    const QString userId =
        conversationList->item(row)->data(Qt::UserRole).toString();
    if (userId == QStringLiteral("phone-user"))
      phoneRow = row;
    if (userId == QStringLiteral("mac-user"))
      macRow = row;
  }
  QVERIFY(phoneRow >= 0);
  QVERIFY(macRow >= 0);
...
void MainWindowLayoutTest::everyNavButtonIsStyledConsistently() {
  ...
  for (const QString &name : navObjectNames) {
    ...
    QVERIFY2(
        styleSheet.contains(QStringLiteral("#%1[selected=\"true\"]").arg(name)),
        qPrintable(QStringLiteral("%1 缺少选中态样式规则").arg(name)));
  }

  // 所有导航按钮左边缘应当一致（同在导航栏布局里，宽度相同）。
  const int width =
      window.findChild<QPushButton *>(navObjectNames.first())->width();
  for (const QString &name : navObjectNames) {
...
void MainWindowLayoutTest::connectionStatusLivesOnNavAvatarWithoutText() {
  ...
  auto *statusDot =
      window.findChild<QLabel *>(QStringLiteral("connectionStatusDot"));
  auto *logoContainer =
      window.findChild<QWidget *>(QStringLiteral("navLogoContainer"));
  ...
  QVERIFY2(!chatHeader->isAncestorOf(statusDot),
           "连接状态仍然占在聊天页右上角");
...
void MainWindowLayoutTest::fileBubbleOffersContextMenu() {
  ...
  app.chatState().receiveFile(QStringLiteral("phone-user"), sourcePath,
                              ...
                              QStringLiteral("text/markdown"), 10);

  MainWindow window(app);
  auto *fileButton =
      window.findChild<QPushButton *>(QStringLiteral("messageFileButton"));
...
void MainWindowLayoutTest::imageBubbleOffersContextMenu() {
  ...
  RemoteIMApplication app(QStringLiteral("desktop-user"), std::move(client));
  app.addContact(QStringLiteral("phone-user"), QStringLiteral("iPhone"));
  app.chatState().receiveImage(QStringLiteral("phone-user"), imagePath, 8, 8,
                               100);

  MainWindow window(app);
  auto *imageLabel =
      window.findChild<QLabel *>(QStringLiteral("messageImageLabel"));
...
void MainWindowLayoutTest::maximizedImageBubbleOpensOnlyOnePreview() {
  ...
  auto *preview = window.findChild<ImagePreviewDialog *>(
      QStringLiteral("imagePreviewDialog"));
...
void MainWindowLayoutTest::
    copyAttachmentToPathCopiesOverwritesAndReportsErrors() {
  QTemporaryDir dir;
  ...
  RemoteIMFileAttachment attachment{sourcePath, QStringLiteral("weekly.md"),
                                    QStringLiteral("text/markdown"), 0};
  ...
  RemoteIMFileAttachment missing{dir.filePath(QStringLiteral("gone.md")),
                                 QStringLiteral("gone.md"), QString(), 0};
  QVERIFY(!MainWindow::copyAttachmentToPath(
      missing, dir.filePath(QStringLiteral("out.md")), &error));
  QVERIFY(!error.isEmpty());

  // 空目标路径：失败。
  QVERIFY(!MainWindow::copyAttachmentToPath(attachment, QStringLiteral("  "),
                                            &error));
  QVERIFY(!error.isEmpty());
}
...
void MainWindowLayoutTest::ctrlShortcutsZoomWholeUi() {
  ...
  auto *chatContentPane =
      window.findChild<QWidget *>(QStringLiteral("chatContentPane"));
...
void MainWindowLayoutTest::
    globalSearchFindsMatchesAcrossConversationsAndJumps() {
  auto client = std::make_unique<FakeRemoteIMClient>();
  RemoteIMApplication app(QStringLiteral("desktop-user"), std::move(client));
  ...
  auto *search =
      window.findChild<QLineEdit *>(QStringLiteral("globalSearchBox"));
  auto *results =
      window.findChild<QListWidget *>(QStringLiteral("globalSearchResults"));
  QVERIFY(search != nullptr);
  ...
  for (int i = 0; i < conversationList->count(); ++i) {
    if (!conversationList->item(i)->isHidden())
  ...
  for (int i = 0; i < results->count(); ++i) {
    if (results->item(i)->text().contains(QStringLiteral("Alice")))
      target = results->item(i);
  }
...
void MainWindowLayoutTest::clickingFilteredConversationJumpsToItsSearchHit() {
  ...
  app.chatState().receiveText(QStringLiteral("peer-b"),
                              QStringLiteral("水管漏了要修"));
  app.chatState().receiveText(QStringLiteral("peer-b"),
                              QStringLiteral("构建失败在链接那步"));
  app.chatState().receiveText(QStringLiteral("peer-b"),
  ...
  for (int row = 0; row < conversationList->count(); ++row) {
    ...
    if (item->isHidden())
      continue;
    if (item->data(Qt::UserRole).toString() == QStringLiteral("peer-b"))
      targetRow = row;
  }
...
QStringList visibleLabels(QListWidget *list) {
  ...
  for (const DirectoryRow &row : readDirectory(list)) {
    if (row.hidden)
      continue;
    labels.append(row.isHeader
                      ? QStringLiteral("[%1 %2]").arg(row.text).arg(row.count)
                      : row.text);
  }
  return labels;
}
...
void MainWindowLayoutTest::groupHeaderCountStaysTheGroupSizeWhileSearching() {
  ...
  QVERIFY(list != nullptr);
  QCOMPARE(visibleLabels(list),
           QStringList({"[同事 2]", "Alice", "Amy", "Bob"}));
...
void MainWindowLayoutTest::videoCoverRetainsPhysicalPixels() {
  ...
  RemoteIMApplication app("preview-user",
  ...
  QVERIFY(button);
  const auto physical =
      (QSizeF(button->iconSize()) * button->devicePixelRatioF()).toSize();
  // Wait for the red decoded cover, not merely the high-DPI placeholder.
  QTRY_VERIFY(button->icon().pixmap(physical).toImage().pixelColor(5, 5).red() >
              200);
...
void MainWindowLayoutTest::messageTextUsesNativeResolutionAndRegularBodyFont() {
  ...
  app.chatState().receiveText(
      "phone-user",
      QStringLiteral("清晰度验证 Native text 123\n\n**强调文字**"));
  MainWindow window(app);
  ...
  for (auto *widget = static_cast<QWidget *>(browser); widget;
       widget = widget->parentWidget())
    QVERIFY2(
        !widget->graphicsEffect(),
        "Message text must not pass through a graphics-effect bitmap cache");
  qInfo() << "message-render-verification" << "dpr" << dpr << "logical"
          << browser->size() << "physical" << capture.size() << "body px"
          << browser->theme().bodyPixelSize;
}
