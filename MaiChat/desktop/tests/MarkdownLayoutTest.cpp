...
MarkdownLayout laidOut(const QString &markdown, qreal width) {
  ...
  layout.layout(MarkdownDocument::parse(markdown), MarkdownTheme::standard(1.0),
                width);
  return layout;
}
...
void MarkdownLayoutTest::typographyDoesNotInheritPlatformApplicationFonts() {
  ...
  const QString source =
      QStringLiteral("正文 and English **强调**\n\n`code 中文`\n\n- List");
...
void MarkdownLayoutTest::codeBlockKeepsEveryLine() {
  ...
  const MarkdownLayout dense = laidOut(QStringLiteral("```\na\nb\n```"), 400);
...
void MarkdownLayoutTest::listIndentsByDepth() {
  ...
  const QString flat =
      QStringLiteral("- 一段不算短的列表项文字，会折行的那种长度");
...
void MarkdownLayoutTest::nestedListsUseCompactDistinctHierarchy() {
  ...
  const MarkdownLayout nested =
      laidOut(QStringLiteral("- x\n  - ") + leaf, 600);
...
void MarkdownLayoutTest::linkHitTestingFindsHref() {
  const MarkdownLayout layout = laidOut(
      QStringLiteral("[点我](https://example.com) 后面是普通文字"), 400);
  ...
  const MarkdownLayout trailing = laidOut(
      QStringLiteral("前面是普通文字 [点我](https://example.com)"), 400);
  bool hitSomewhere = false;
  for (int x = 4; x < 400; x += 4) {
    if (!trailing.linkAt(QPointF(x, 8)).isEmpty())
      hitSomewhere = true;
  }
  ...
  const MarkdownLayout wrapped = laidOut(
      QStringLiteral(
          "[a very long link label that certainly wraps onto another line]"
          "(https://example.com)"),
      200);
  QVERIFY(wrapped.height() > 24); // 确实折了行
  QCOMPARE(wrapped.linkAt(QPointF(8, 8)),
...
void MarkdownLayoutTest::paintsWithoutTouchingOutsideTheClip() {
  ...
  QImage image(400, static_cast<int>(layout.height()) + 8,
  ...
  below.fill(Qt::white);
  {
    ...
    layout.paint(&painter, QPointF(0, 0),
                 QRectF(0, layout.height() + 100, 400, 40));
  }
...
void MarkdownLayoutTest::deepIndentStopsGrowing() {
  ...
  const auto leafHeight = [](int depth, const QString &leaf) {
    ...
    for (int level = 0; level < depth; ++level) {
      source += QString(level * 2, QLatin1Char(' ')) +
                QStringLiteral("- a%1").arg(level) + QLatin1Char('\n');
    }
    source +=
        QString(depth * 2, QLatin1Char(' ')) + QStringLiteral("- ") + leaf;
  ...
  const QString leaf =
      QStringLiteral("一段够长的叶子节点文字用来看还剩多少宽度可用啊");
