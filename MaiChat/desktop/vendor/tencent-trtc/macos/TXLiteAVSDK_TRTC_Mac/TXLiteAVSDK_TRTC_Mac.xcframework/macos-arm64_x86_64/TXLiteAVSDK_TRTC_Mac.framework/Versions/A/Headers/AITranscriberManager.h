...
///@note Supported languages are as follows (contact us if you need support for other languages):
/// "zh"(Chinese), "en"(English), "vi"(Vietnamese), "ja"(Japanese), "ko"(Korean), "id"(Indonesian),
/// "th"(Thai), "pt"(Portuguese), "tr"(Turkish), "ar"(Arabic), "es"(Spanish), "hi"(Hindi), "fr"(French),
/// "ms"(Malay), "fil"(Filipino), "de"(German), "it"(Italian), "ru"(Russian), "sv"(Swedish), "da"(Danish), "no"(Norwegian).
...
/// @note If you need to translate the transcription results into other languages, set the target language codes here.
///@note Supported languages are as follows (contact us if you need support for other languages):
