...
class ITRTCCloudCallback {
   public:
     ...
     * @param localQuality Upstream network quality of the local user.
     * @param remoteQuality Upstream network quality of remote users.
     */
 ...
 */
     ...
     *
     * Triggered when screen sharing is resumed via {@link resumeScreenCapture}.
     * @param reason The reason for resuming:
     * - 0: The user manually resumed the screen sharing.
     * - 1: Resumed because the shared window became visible again (macOS) / Resumed automatically after screen sharing parameters were set (Windows).
     * - 2: Resumed because the shared window was restored from minimized (Windows only).
     * - 3: Resumed because the shared window was restored from hidden (Windows only).
     * - 4: Resumed due to a system operation (iOS only).
     */
