...
int main() {
  ...
  server.Post("/chat/completions", [](const httplib::Request &request,
    ...
    if (input == "error") {
      response.status = 429;
      response.set_content(R"({"error":{"message":"quota exhausted"}})",
                           "application/json");
      return;
    }
    if (input == "write") {
      ...
      response.set_content(
          frame({{"tool_calls", Json::array({invocation})}}, "tool_calls") +
              "data: [DONE]\n\n",
          "text/event-stream");
      return;
    }
    ...
    response.set_chunked_content_provider(
        "text/event-stream",
        [slow, step = 0](size_t, httplib::DataSink &sink) mutable {
          std::string text;
          if (step == 0)
          ...
          if (!sink.write(text.data(), text.size()))
            return false;
          if (step++ >= 2) {
            sink.done();
            return false;
          ...
          std::this_thread::sleep_for(
              std::chrono::milliseconds(slow ? 1600 : 160));
          return true;
        });
  });
  ...
  try {
    ...
    CHECK(failure["error"].get<std::string>().find("quota exhausted") !=
          std::string::npos);
    auto messageCount = failure["messages"].size();
    maiMobileAgentDestroy(agent);
    agent = maiMobileAgentCreate();
    CHECK(call(agent, config)["ok"] == true);
    CHECK(snapshot()["messages"].size() == messageCount);
    ...
    std::puts("PASS: streaming, part kinds, permissions, interruption, "
  ...
  listener.join();
  for (const auto *file :
       {"sessions.db", "sessions.db-shm", "sessions.db-wal", "result.txt"})
