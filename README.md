...
> 本地优先的 AI CLI 工作台，并支持从 IM 远程操控本机 AICLI：在桌面端围绕本地仓库组织任务、终端、代码审查、构建运行，调用你本机真实安装的 `codex` / `opencode` / `claude` 完成开发；离开电脑时，用手机或另一端 IM 直接给这台机器上的 AICLI 发任务、切模型、看 diff、收结果。
...
1. **本地 AI CLI 工作台**：在桌面端打开一个仓库，用真实 PTY 终端驱动本机 AICLI，围绕它组织任务、定时任务、代码审查、仓库分析、构建运行与日志。
2. **IM 远程操控 AICLI（核心特性）**：每台装了本应用的机器登录一个 IM 身份；你在手机或另一端 IM 里把它当成一个"聊天联系人"，直接给这台机器上的当前 AICLI 发任务、发图片/语音、切模型、看 diff、中断或压缩上下文，AICLI 的结果会清成聊天友好的 Markdown 回传给你。多台机器就是多个 IM 联系人，各自独立驱动。

下文的 AICLI 指可以从命令行启动的 AI 编程工具，例如 `codex`、`opencode` 和 `claude`。当前默认推荐使用 Codex；`opencode` 亦为一等支持；Claude Code 保留为可选项，但不建议作为默认 CLI。
...
- 从 iOS App 或 Qt 桌面 IM 客户端把文本、图片、语音、任务或斜杠控制命令发送到目标机器的当前 AICLI，并收取回传结果。

## 为什么使用它

- 仓库上下文集中：任务文档、构建命令、运行命令、审查批注和分析记忆都围绕当前仓库组织。
- AI CLI 不被封装黑盒：应用启动的仍然是你本机真实安装、已登录的 `codex` / `opencode` / `claude`。
- 开发工作流闭环：从任务描述、AI 会话、diff 审查到构建运行，都能在一个桌面应用内衔接。
- 随时随地远程操控：离开电脑时，用手机或另一端 IM 就能给任意一台机器的 AICLI 派活、切模型、看改动、收结果，多台机器逐一加为联系人即可分别驱动。
- 数据本地优先：项目数据、消息记录和运行记录默认保存在本机或目标仓库目录，不经中转服务器。
...
远程 IM 基于应用内 IM 通道：每台机器登录一个 IM 身份，任意 IM 客户端（iOS App、Qt 桌面 IM 客户端，或 AICLI 通过 `imcli`）把它当联系人发消息，即可驱动这台机器上的当前 AICLI 并收取回传。除了直接发任务文本，还支持一组斜杠控制命令：

| 命令 | 作用 |
| --- | --- |
| `/status` | 查看当前 AICLI 运行状态 |
| `/plan` · `/build` | 切换 AICLI 计划 / 执行模式 |
| `/models` · `/model <序号\|ID\|档位>` | 查看 / 切换模型或 Codex 推理档位 |
| `/goal [目标\|clear\|pause\|resume]` | 查看 / 设置 / 管理当前 Goal |
| `/btw <任务>` | 启动子 Agent 处理任务，完成后单独 IM 回传 |
| `/diff [--stat] [路径]` | 发送当前仓库未提交改动的 Diff |
| `/interrupt` | 中断当前正在执行的任务 |
| `/compact` · `/clear` | 压缩上下文 / 清空并开启新会话 |
| `/help` | 查看可用控制命令 |

- 控制命令对 `codex` / `opencode` 走源码级 IPC 通道（每条消息带 ack/重发，回传丢一次会自动补，不会"一丢就一直丢"）；`claude` 目前仅支持任务注入、`/status` 与 `/diff`。
- 只有在联系人白名单里的账号才能下发任务与命令；桌面端不直接执行远程发来的 shell 命令，远程消息只作为输入进入当前 AI 会话。

### 桌面端、iOS 与语音

远程 IM 用于把手机或另一端的消息转发到桌面端当前 AICLI。
...
- 远程消息只会作为 AICLI 输入进入当前会话，不提供远程任意命令执行入口（但它驱动的 AICLI 本身会在目标仓库内执行 shell 与改文件，安全边界取决于所选 CLI 的沙箱/审批设置）。
- 一台机器对应一个 IM 账号，远程消息只会进入当前前台项目的会话；多台机器各自作为独立联系人操作。
- 远程控制命令能力依 CLI 而异：`codex` / `opencode` 完整，`claude` 仅支持任务注入、`/status` 与 `/diff`。
