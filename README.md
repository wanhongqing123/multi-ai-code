...
### 6. 习惯学习智能体（Skill 自动建议）

顶栏新增 `🎓 Skill 学习` 入口，对应一个被动观察 + 定期汇总的智能体。它会：

- 在你使用本软件期间，静默采集 7 类输入侧行为（主会话 prompt、仓库查看 prompt、Diff 批注、代码标注、模板调用、方案导入）
- 每 24 小时跑一次本地聚合，把重复模式拢成簇
- 把每个簇的**最小聚合摘要**（≤ 5 条样本 + 计数 + 项目数）交给主会话 AI 同款 CLI 生成 prompt 模板
- 在「候选」tab 列出生成结果，你来决定采纳 / 编辑 / 暂不处理 / 丢弃
- 采纳后的模板直接进入顶栏「📋 模板」列表，复用现有调用路径

数据边界：

- 所有原始事件存本机 SQLite，默认保留 90 天（可改 30 / 60 / 180）
- 不采集 AI 返回、终端输出、文件内容、方案正文
- 不监控软件之外的系统活动
- 「采集」tab 提供主开关、各事件类型子开关、最近 100 条事件查看、一键清空
- 如果本机没有 AI CLI 或调用失败，自动降级为纯本地启发式候选

首次升级到本版本时会弹一次告知面板，可在那里直接选「先关闭采集」。

### 7. 项目知识库（自动积累 · 可搜索）

顶栏新增 `🧠 项目记忆` 入口。这是一个**随项目使用频次自动渐进积累**的本地知识库：

- 智能调度后台跑：用户空闲 15 分钟 / 累计 5 条新提示 / 距上次满 24 小时，三个条件任一即触发
- 跑总结时**不打断主 AICLI**：完全独立子进程，且与 habit 生成器共享串行队列（不会同时撞 OAuth）
- 数据源全部来自已有信号（git log / 最近修改文件 / 用户 prompts / 既有 KB），不额外采集
- 三段存储自动控量：`hot` 30 条 / `warm` 30 主题 / `cold` 5 段 / `pinned` 不限；超额 AI 自动合并降级
- 硬上限 500 KB；面板里能看实时占用、手动压缩、清空、编辑、⭐pin

**AI 怎么用上这份知识库**：

- 通用兜底：digest（≤ 2 KB 紧凑总览）自动注入 system prompt，Claude / Codex 两端都吃得下
- Claude Code 增强：通过临时 MCP 配置暴露 `query_kb / list_topics / get_topic` 三个工具，AI 觉得 digest 不够细就主动按需拉
- Codex 端仅 digest（OpenAI CLI v1 暂未支持 MCP）

**面板四个 tab**：总览（digest + tier 占用 + 下次调度状态 + 立即总结/压缩） / 主题（分 tier 列出，可编辑/pin/删） / 搜索（FTS5 全文） / 设置（清空 + 调试信号）
