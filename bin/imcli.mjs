...
Command details:
  ...
  imcli send-file <user> <filePath>
    Send a local file of any type to one user (up to 20GB).
  ...
  imcli send-video <user> <videoPath>
    ...
    because the IM SDK only accepts these.
    The cover image is generated by the IM service after upload, so you do not
    supply a thumbnail. Large videos take a while: the command returns as soon as
    the desktop app accepts the job, and the upload continues in the app.
    Use send-file instead when the receiver should get a plain attachment card.
  ...
  imcli broadcast <user1,user2> --text-b64 <base64>
    ...
    This is text-only. Use send-image, send-file or send-video separately for attachments.
...
File notes:
  ...
  send-file accepts any regular file up to 20GB; md/markdown/html/htm render with
  tap-to-preview on receivers, other types show a save-to-local file card.
  send-video accepts local mp4 and mov files up to 20GB and renders as a playable video.
  forward sends the source message text only; it does not re-send image, file, or video
  attachments.
...
async function main(argv) {
  ...
  if (command === 'send-video') {
    const [toUserId, ...pathParts] = args
    const localPath = pathParts.join(' ').trim()
    if (!toUserId || !localPath) throw new Error('usage: imcli send-video <user> <videoPath>')
    const value = await requestJson('POST', '/send-video', { projectId, toUserId, localPath })
    console.log(`sent video to ${value.toUserId}`)
    return
  }
