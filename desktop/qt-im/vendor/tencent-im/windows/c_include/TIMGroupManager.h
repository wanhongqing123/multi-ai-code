...
/**
 * 8.4 按照群维度“获取每个群的未决列表的已读信息”
 *
 * @param json_group_getpendency_readinfo_param  是 Json 字符串格式的请求参数, Json Key 请参考 @ref GroupPendencyReadInfoOption
 * @param cb 获取群未决列表已读信息成功与否的回调。回调函数定义和参数解析请参考 @ref TIMCommCallback
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 @ref TIMResult
 *
 * @note
 * - kTIMGroupPendencyReadInfoOptionStartTime 设置拉取时间戳, 第一次请求填 0, 后边根据 server 返回的 @ref GroupPendencyReadInfoResult 键 kTIMGroupPendencyReadInfoResultNextStartTime 指定的时间戳进行填写。
 *
 * __示例__
 * @code{.cpp}
 * Json::Value get_pendency_readinfo_option;
 * get_pendency_readinfo_option[kTIMGroupPendencyReadInfoOptionStartTime] = 0;
 * int ret = TIMGroupGetPendencyReadInfo(get_pendency_readinfo_option.toStyledString().c_str(), [](int32_t code, const char* desc, const char* json_param, const void* user_data) {
 *     Printf("TIMGroupGetPendencyReadInfo code:%d|desc:%s|json_param %s\r\n", code, desc, json_param);
 * }, nullptr);
 *
 * // get_pendency_readinfo_option.toStyledString().c_str() 得到json_group_getpendency_readinfo_param JSON 字符串如下
 * {
 *    "group_pendency_read_info_option_start_time" : 0,
 * }
 *
 * @endcode
 */
TIM_API int TIMGroupGetPendencyReadInfo(const char* json_group_getpendency_readinfo_param, TIMCommCallback cb, const void* user_data);

/**
 * 8.5 对指定群的未决列表设置已读
 *
 * @param group_id 群组 ID
 * @param cb 对群的未决列表设置已读成功与否的回调。回调函数定义请参考 @ref TIMCommCallback
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 @ref TIMResult
 *
 */
TIM_API int TIMGroupSetPendencyReaded(const char* group_id, TIMCommCallback cb, const void* user_data);
...
// 13.5 GroupPendencyReadInfoOption(获取群未决列表已读信息的参数)
// uint64, 只写(必填), 设置拉取时间戳, 第一次请求填0, 后边根据server返回的 @ref GroupPendencyReadInfoResult 键 kTIMGroupPendencyReadInfoResultNextStartTime 指定的时间戳进行填写
static const char* kTIMGroupPendencyReadInfoOptionStartTime = "group_pendency_read_info_option_start_time";

// 13.6 GroupPendencyReadInfo(群未决列表已读信息定义)
// string, 只读, 群组ID
static const char* kTIMGroupPendencyReadInfoGroupId = "group_pendency_read_info_group_id";
// uint64, 只读, 已读时间戳（毫秒）
static const char* kTIMGroupPendencyReadInfoReadTimeStamp = "group_pendency_read_info_read_timestamp";

//------------------------------------------------------------------------------
// 13.7 GroupPendencyReadInfoResult(获取群未决列表已读信息的返回)
// uint64, 只读, 下一次拉取的起始时间戳,server返回0表示没有更多的数据,否则在下次获取数据时以这个时间戳作为开始时间戳
static const char* kTIMGroupPendencyReadInfoResultNextStartTime = "group_pendency_read_info_result_next_start_time";
// array @ref GroupPendencyReadInfo, 只读, 群未决列表已读信息
static const char* kTIMGroupPendencyReadInfoResultReadInfoList = "group_pendency_read_info_result_read_info_list";
