...
协议解析由 `electron/remote-im/replyProtocol.ts` 统一负责。Claude 和 Codex 必须共用这层协议解析；不同 AICLI 只允许在输出来源、提交方式和界面显示上做适配。

## 消息提交与显示

- Codex 的普通 IM 消息必须通过源码控制桥提交，不能依赖 PTY 模拟输入。
...
- AICLI 回给 Electron 的结构化输出保留完整回复标记；Codex TUI 渲染时只显示标记内正文，且不能在流式输出阶段闪现半截标记。
