...
本仓库只保留一个清晰的维护入口，避免 Codex / OpenCode / Claw 子仓分支混乱。
...
- Claw 子仓：`third_party/aicli/claw` 使用 `multi-ai/im-bridge`

`.gitmodules` 已固定三个子仓的跟踪分支为 `multi-ai/im-bridge`。

Claw 的 `multi-ai/im-bridge` 目前与上游 `main` 同一个提交（还没有任何定制），
建它是为了让第二步的定制有地方落，同时不破坏「子仓只用这一个分支」这条规则。

## 开发规则

1. 主仓开发只在 `main` 上进行。
2. Codex / OpenCode / Claw 的 Multi-AI Code 定制改动只提交到各自子仓的 `multi-ai/im-bridge`。
3. 不再用 `fix/*`、`feature/*` 等临时分支承载长期定制功能。
4. 如果为了临时验证创建了实验分支，最终必须把有效提交合入 `multi-ai/im-bridge`，并让主仓 `main` 更新 submodule 指针。
5. 主仓提交 submodule 指针前，必须确认各子仓对应提交已经推送到远端。
