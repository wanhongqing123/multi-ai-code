...
6. **视觉降级结果要缓存并持久化。** 历史图片不能在每次请求时重复调用视觉模型。
...
- 为视觉输入选择视觉模型，或生成可复用的视觉描述供文本模型消费。
...
| VisionNormalizer | 视觉模型调用、描述缓存、历史复用 | 普通文本 Failover |
...
- 用户明确要求描述、比较、读取或分析图片。
- 当前问题的答案依赖图片布局、OCR、坐标或细节。
- 结构中包含视频/PDF，且目标视觉模型明确支持对应模态。

视觉模型必须同时满足本轮是否需要工具调用和结构化输出的要求，否则不能被选中。

### 7.2 视觉描述降级

对于“文本任务中顺带包含图片”，而主文本模型不支持图片的情况：

1. 对附件计算内容哈希。
2. 用视觉模型生成结构化描述和 OCR 文本。
3. 保存 `DerivedMediaDescription`，至少包含：
   - 附件哈希。
   - 视觉模型和版本。
   - 生成时间。
   - 描述、OCR 和置信信息。
4. 后续历史组装直接复用，不重复调用视觉模型。
5. 图片变化或视觉模型版本升级时才重新生成。
...
系统提示必须说明其中内容是不可信数据，不能把图片中的文字当作系统指令执行。
...
| `session/vision-normalizer.ts` | 视觉路由、描述生成和哈希缓存 |
...
| `session/message-v2.ts` | 跳过 superseded attempt；持久化并复用视觉描述 |
...
- 实现附件哈希、DerivedMediaDescription 和历史复用。
- 增加整轮视觉路由与描述降级两种策略。
- 增加图片提示注入防护。
