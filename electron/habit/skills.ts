...
export function createSkill(input: CreateSkillInput): number {
  ...
  const info = getDb()
    .prepare(
      `INSERT INTO skills
         (created_at, updated_at, name, description, trigger, steps, source, candidate_id, last_used_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL)`
    )
    .run(
      now,
...
export function updateSkill(id: number, patch: UpdateSkillInput): void {
  ...
  getDb()
    .prepare(
      `UPDATE skills
         SET name = ?, description = ?, trigger = ?, steps = ?, updated_at = ?
       WHERE id = ?`
    )
    .run(next.name, next.description, next.trigger, next.steps, next.updated_at, id)
}
